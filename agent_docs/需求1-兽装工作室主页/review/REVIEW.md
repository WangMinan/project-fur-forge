...
尚未进入正式阶段 5；阶段 4 进行中。T01–T21、GATE-06、GATE-07、EXT-01 与 EXT-02 已完成，阶段 C 已解锁，下一任务为 T22。
...
- T22 的数据库列已部分存在，但共享请求/响应、管理 CRUD、领养矩阵、排序/精选和前端仍未完整接入；不得把“表里有列”误判为完成。
- T22 只处理常规领养基础字段；展会实体、当前展会和完整掉落状态留到 T37，避免 P0/P1 边界漂移。
- T23–T25 必须保持 `design_sheet`、`studio_photo` 与首页资产角色分离，并继续使用活动居中水印。
...
- T22：完整作品字段、常规领养基础矩阵、排序/精选和公开泄漏边界；
...
- 尚未执行阶段 5 子代理总评审。
- 历史 Reader Test 和 T21 独立审查结果保留在 `implementation/notes/`；后续任务不得改写其当时结论。