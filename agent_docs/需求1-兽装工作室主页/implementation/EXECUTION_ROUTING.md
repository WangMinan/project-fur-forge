...
- 所有代码、文档、测试和修复从最新 `origin/main` 建立独立任务分支，并通过 PR 合入 `main`；
- 禁止直接在 `main` 提交或推送；PR 的相关 GitHub Actions 与必要 Review 完成后才能合并；
- 不 force push、不硬 reset、不重写已验收历史；只读分析可以并行，任何写入必须串行；
...
4. T52-E1～E6 生产能力开发；
5. T51-F3 低分辨率出厂照适配；
6. T51-F4 管理端 FFmpeg 可见进度与作品竖图水印；
7. T51-F5 竖版详情水印与作品卡/后台预览对齐；
8. T51-F6 公共页脚备案居中与三栏重排；
9. T51-F7 管理私有预览与首页 Hero 4K 配方；
10. T51-F8 全局水印流程与首页精选排序优化；
11. T49-R1 最新 SHA CI + 独立综合 Review；
12. T50 最终回归；
13. GATE-E 代码冻结；
14. T53-F1～F5 用户与远程机执行。
...
- T51-F3 出厂照私有 FFmpeg 适配源、公开配方输入与 publication operation 恢复；
- T51-F4 内嵌 FFmpeg 异步子进程、`recipe-v3` 不可变身份与竖图单居中水印缩放；
- T51-F5 竖版详情与作品卡统一水印参考宽度、不可变身份与回归测试；
- T51-F8 组合 profile 保存/刷新、空库激活、精选完整集合事务重排与公开前 12 投影；
...
- T51-F3 出厂照区/发布区非阻断提示与真实发布流程；
- T51-F4 管理端全部 FFmpeg 入口的动态等待进度、阶段/等待时间与浏览器回归；
- T51-F5 后台公开水印预览与竖版详情/3:4 卡片视觉一致性回归；
- T51-F6 公共页脚中间备案区、均衡列间距、右侧两行法务文案和响应式布局；
- T51-F7 管理端 320/640 私有预览、显式原图入口、Hero v2 SourceSet 与浏览器请求验证；
- T51-F8 branding 单流程布局、作品双 Tab、精选移动/状态和三视口交互回归；
- T52-E4 下架/刷新状态；
