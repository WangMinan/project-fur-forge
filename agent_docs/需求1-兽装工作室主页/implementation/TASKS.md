...
> **角色**：PLAN 的唯一可勾选实施分解。
> **状态**：T01–T18、`GATE-06` 与 EXT-02 已完成；T19 未启动。任务编号保留 T01–T53，T09 之后按垂直切片排序。

## 当前目标

T01–T18、`GATE-06` 与 EXT-02 已完成。下一批从 T19 开始，以 T19–T21 补齐公开 SSR、首页横竖双源轮播与第一垂直切片门禁。随后完成其余 P0、P1；P2 不阻塞核心上线。
...
- [x] **GATE-06 · T13 认证前端接线**：Kimi 接入登录、Session、退出和改密接口；CSRF token 只保存在页面内存；未认证时管理界面只显示登录；在真实浏览器验证 Host-only/Secure/SameSite Cookie、公开/管理域隔离、Session 恢复、退出/改密失效，以及成功和错误响应的 `no-store`/`noindex`。只修改认证相关前端与浏览器测试，不进入上传、CRUD 或媒体处理。_依赖：T13。2026-08-01：用户验收通过，见 `notes/T13-AUTH-UI-2026-07-31.md`。_
...
- [x] **T02 · 运行配置、Host 与安全日志工具**：版本化配置、优先级、Host 隔离、日志脱敏工具和秘密扫描；2026-07-31 追加收口非测试 origin，并将“测试之外不得硬编码任何配置具体值”固化为全局工程门禁，本机值只保存在 `.env`。证据：`notes/T02-T03-2026-07-29.md`、`notes/T02-ORIGIN-ENV-CLOSURE-2026-07-31.md`。
...
- [x] **T13 · 唯一管理员最小认证**：幂等初始化、登录、退出、改密、受保护命令重置、SessionVersion、锁定、Host/Origin/CSRF 和集成测试；不做邮件找回。_依赖：T12。2026-07-31：唯一管理员服务端认证通过；S2 Review 补齐私有响应 no-store、显式迁移前置和隐藏 TTY 密码输入；前端接线随后由 GATE-06 收口，见 `notes/T13-AUTH-2026-07-31.md`、`notes/S2-REVIEW-CLOSURE-2026-07-31.md` 与 `notes/T13-AUTH-UI-2026-07-31.md`。_
- [x] **T14 · 角色化私有原图条件直传**：上传会话必须声明作品/站点归属与媒体角色；不可预测 Key、5 分钟 V4 PUT、固定 MD5/SHA-256/Content-Type/禁止覆盖，浏览器直传 `project-furry-forge-private`；同一关系不得静默变更角色。_依赖：T10、T13、GATE-06。2026-08-01：当前视口内的摘要/PUT 进度/校验提示与真实接线通过用户联合验收。_
- [x] **T15 · 上传完成校验与用途预览数据**：HEAD/图片信息验证大小、格式、摘要与像素；首页横版校验宽大于高、竖版校验高大于宽；保存 EXIF 修正后的焦点、3:4/16:9/9:16/原比例或 contain 参数及水印安全角；失败可读并精确清理。_依赖：T14。2026-08-01：真实上传、核验、预览与错误反馈通过用户联合验收。_
- [x] **T16 · `recipe-v1` 跨 Bucket 衍生图与基础水印**：对超过 OSS 20 MB 输入上限的合规原图先用内嵌 FFmpeg 生成私有处理源，再生成 work-card、home-hero-landscape、home-hero-portrait、design-sheet、detail 的受限宽度，WebP + 一种 fallback；使用 OSS 烘焙 `brand-standard-v1` 水印并 `sys/saveas` 到 `project-furry-forge-public`；Logo 摘要/profile/锚点进入 identity，私有原图保持无水印，验证 OSS 为公开 variant、最终格式和水印的唯一配方权威。_依赖：T15、EXT-02。2026-08-01：标准水印按最终输出宽度的 18% 生成，不再按高分辨率处理源放大；用户联合验收通过。_
- [x] **T17 · 最小作品创建与编辑**：后台创建一件非领养作品，维护基础信息、联系人、短属性、出厂照和草稿；联系人不出现在公开投影；媒体区明确为“出厂照”，不使用通用图片角色；P0 仅允许永久删除未发布作品，清理公开衍生图但保留私有原图，T40 再替换为 30 天回收站。_依赖：T16、T08。2026-08-01：作品编辑、出厂照关系、保存基线与受限删除通过用户联合验收。_
- [x] **T18 · 双 Bucket 发布与下架操作**：记录操作意图，生成并验证带水印公开衍生对象，提交 SQLite 公开引用；下架先隐藏公开投影再删公开对象；不修改 Object ACL。_依赖：T17。2026-08-01：发布内置一次幂等 OSS 补偿重试，管理端显示已生成/总数/剩余进度；真实作品 3 张出厂照均具备 12 个公开 variant，单次发布与下架通过用户联合验收。_
