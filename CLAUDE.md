...
- QQ/QQ 群直达链接从已上传的官方二维码在服务端派生；只接受精确的 `https://qm.qq.com/q/*` 结果，不在管理端新增手填链接。
- 首页固定为品牌 Hero、代表作品、自设委托、设定领养四幕；首页领养按现有排序投影最多三项 `available` 并提供切换，不足三项按真实数量退化。
- 首页代表作品按管理端顺序最多投影 5 件，每件仍必须有 READY 竖版出厂照。
- 全局领养营业状态已退役；委托营业状态只保留“开放 / 暂停”和一条状态标签，不再维护独立短说明。
