...
阶段 4 · IMPLEMENTATION 进行中。T01–T21、GATE-06、GATE-07、EXT-01 与 EXT-02 已完成；T22 后端已完成，等待前端接线、独立浏览器 Review 和用户验收。T22 仍为 `[ ]`，不得进入 T23。
...
T22 后端差异、兼容策略、接口和验证见 [`implementation/notes/T22-BACKEND-2026-08-03.md`](./implementation/notes/T22-BACKEND-2026-08-03.md)。
...
- T22 三用途写入联合类型、完整管理读写、人工排序/精选、历史展会只读兼容和公开精选 6 项上限；
...
- 2026-08-03：T22 后端完成；现有 11 项迁移已满足列与约束，未制造新迁移；下一步为前端接线，T22 保持未勾选。

## 下一步

按以下顺序继续 T22：

1. 用户选择本任务的前端 Agent，按 [`implementation/notes/T22-BACKEND-2026-08-03.md`](./implementation/notes/T22-BACKEND-2026-08-03.md) 接入创建、编辑、列表、排序、精选和错误状态；
2. GPT-5.6 Sol 使用新的上下文做管理 Host / 公开 Host 浏览器与视觉 Review；
3. 用户确认后勾选 T22，才能进入 T23。
