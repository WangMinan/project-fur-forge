...
> **状态**：2026-08-20 阶段 E 文档 Review 与空上下文读者测试后修订；无未答 OQ。
...
### 2026-08-20 阶段 E 执行窗口

T04～T34、T34-F1 与 M01～M11 已完成当前工程实现。T35/T36 中 Linux 发布镜像 runtime registry、容器嵌入和 Docker Hub 分发核验继续后置到部署阶段，因此 GATE-D 仍未关闭；它们不阻塞 T37～T47 的本地设计、实现和人工视觉迭代，但在最终独立 Review、镜像冻结和生产发布前必须补齐。
...
A 组件与进度地基
  ...
  → E 动效、Hero 焦点与首页四幕
  → F 最终 Review、人工验收、release smoke 与发布
```
...
- T38 先让 Hero、lead representative work、非对称自设委托和单项 available 领养在桌面、平板、手机静态成立。
- 领养从章节起点进入目标视口后，同屏包含标题、角色图、名称/物种、状态和唯一行动；图片不因全宽 16:9 把 caption 推到下一屏。
- 内容 SSR/无 JavaScript 默认可见；删除所有章节共用同一种上浮 reveal 的假设。

### E3. Hero 焦点与裁切

- T39 固化未启用 item 的 CAS/focal 写入、共享 asset 冲突阻断和不可变变体重建。
- T40 提供横/竖目标画框、九宫格预设与任意坐标最近提示，继续复用四集合独立管理 UI。
...
- T43 代表作品：lead 大图、短 caption、一个行动和低权重次级浏览；次级作品不逐卡 reveal。
- T44 自设委托：图片先建立章节，文字轻错峰，一个主行动；不让整个章节卡片浮起。
- T45 单项领养：一次媒体揭示和 caption 交接，保持一屏完整表达；不使用商品卡阴影、tilt 或循环呼吸。
