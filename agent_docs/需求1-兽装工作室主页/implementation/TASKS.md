...
> **角色**：PLAN 的唯一可勾选实施分解。
> **状态**：T01–T22、GATE-06、GATE-07、EXT-01 与 EXT-02 已完成。T23–T25 实现、自动化自检和用户 2026-08-04 人工核验已通过；独立 Review 尚未执行，因此 T23、T24、T25 均保持未勾选。任务编号保留 T01–T53。

## 当前目标

从已经通过人工验收的第一件作品垂直切片，扩展到 P0 可部署核心。T22 已完成三用途作品字段、管理体验、公开投影、独立 Review 和用户验收；T23–T25 当前已完成工程实现、自动化自检和用户人工核验，等待新的独立 Review。T25 不在本轮自行勾选；T37 展会矩阵仍未实现。T22 收口证据见 [`notes/t19-t22/T22-INDEPENDENT-REVIEW-2026-08-03.md`](./notes/t19-t22/T22-INDEPENDENT-REVIEW-2026-08-03.md)。
...
- [ ] **T23 · 多图关系、角色约束与配方按需生成**：最多 5 张出厂照、1 张领养设定图；分角色顺序/主图/完整画布；同一资产不得冒充多个角色；只生成页面实际需要的 recipe，使用当前活动水印 profile。**实现、自动化与用户人工核验已通过，等待独立 Review。** _依赖：T22。_
- [ ] **T24 · 管理端作品列表与角色化快速编辑**：按“设定图/出厂照”分区，维护顺序、主图、必要焦点/裁切，显示当前活动水印、已保存原图和真实公开预览；点击发布前自动保存页面当前修改，不恢复四角锚点。**实现、自动化与用户人工核验已通过，等待独立 Review。** _依赖：T23。_
- [ ] **T25 · 横版领养设定图与常规领养发布**：领养发布要求 1 张横版完整设定图；`/adoptions` 使用设计图主视觉，统一 `/works/{slug}` 详情分开“设定图”和“出厂照”；只有设定图的领养不进入 `/works`；`recipe-v2` 设定图使用不重叠的左右双水印；不提前建设展会掉落完整矩阵。**实现、自动化与用户人工核验已通过，等待独立 Review；本轮保持未勾选。** _依赖：T23。_
