...
> **角色**：当前媒体公开行为的唯一事实源。
> **最后校准**：2026-08-09。
> **状态分层**：媒体配方、阶段 D 页面和第 7 节 Endpoint 分离已经落地；第 6、8～9 节的私有/BPA preflight、ESA 托管私有 OSS 回源和精确缓存撤销仍须在阶段 E 的 T52-E2～E4 完成。阶段 F 不再改变媒体产品契约，主要执行真实云配置与远程验证；所需独立运维辅助脚本按 TASKS 的 F 边界处理。尚未落地前不得描述为当前代码事实。
