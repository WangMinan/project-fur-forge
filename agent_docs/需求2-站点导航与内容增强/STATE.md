...
T17 第三轮用户反馈新增四组明确修复：作品目录删除访客总数；自设委托与掉落领养恢复为一级导航；领养卡片将物种、展会、时间和价格压缩为可换行信息带；`/admin/site/content` 的默认委托、关于、条款、隐私和防诈骗文案改为直接面向访客，删除内部维护口吻。历史迁移不重写，管理员已经修改的字段不覆盖。

T17-F3 已完成：作品筛选条不再接收/渲染访客总数；`PUBLIC_NAV_ITEMS` 直接提供两个业务一级入口，768 px 使用移动菜单且 1024 px 桌面导航无重叠；领养信息带与 32 px 网格行距把实际三视口图片行距收敛为 102/162/102 px。迁移 `0033_requirement_2_visitor_copy.sql` 只替换仍等于旧默认值的字段，管理员自定义保留；公开隐私页不再出现内部维护句。相关 migration integration 17/17、公开 E2E 32/32、后台 E2E 1/1、unit 179/179、lint/typecheck/build/verify 和实际 Chrome 三视口均通过，证据见 `implementation/notes/T17-F3-NAV-ADOPTION-COPY-2026-08-14.md`。

T17 第四轮用户反馈要求把公开导航和 `/adoptions` 页面标题统一为“设定领养”，并让 `/works` 搜索/筛选区到首图的间距与另外两个目录一致。实施前本地 Chrome 实测 `/works` 为 64 px，`/adoptions` 与 `/returns` 均为 32 px；T17-F4 将复用既有 `var(--space-6)` 并用三视口跨页几何断言锁定。
...
- 公开导航的当前目标为直接显示“自设委托”和“设定领养”两个一级入口；“关于我们”继续使用既有下拉，桌面、移动和页脚仍复用同一数据源。
- “设定领养”统一 `/adoptions` 的公开导航、H1、SEO 和分页可访问名称；页面仍同时展示常规领养与展会掉落，没有改变筛选、数据模型或发布语义。
- 三个公开目录的搜索/筛选区到首图统一使用 `var(--space-6)`，三个固定视口实测均为 32 px。

## 当前约束

- 只按 TASKS 串行实施；当前 `GATE-01`、T01～T16、T15-F1～T15-F4 与 T17-F1～T17-F4 已勾选，T16-R1 与 T17 保持开放。
