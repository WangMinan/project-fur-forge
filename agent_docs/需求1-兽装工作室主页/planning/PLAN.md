...
- 当前管理导航固定为“首页管理 → 文案配置 → 全局水印 → 作品管理 → 修改密码”，五个入口的导航、一级标题和标签页标题同名；首页管理内部增加“首页大图 / 委托页大图”Tab，复用专用双源上传、排序、启停、预览和发布链路。横/竖槽位显示原图尺寸；低于推荐尺寸时以持续风险提示和确认对话框启动 FFmpeg 适配，不用通用 CMS 或第二套上传器。委托/领养营业状态及 T26–T27 固定文字仍留在独立“文案配置”，不建设万能 CMS、页面树或账号列表。
