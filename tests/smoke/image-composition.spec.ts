...
test('R6 saves independent crops and display choices, then renders square thumbnails at six viewports', async ({ page }) => {
  ...
  for (const reducedMotion of ['no-preference', 'reduce'] as const) {
    ...
    // A valid DTO alone is insufficient: the visible image and selection must agree.
    await expect.poll(() => dialog.evaluate(el => {
      const canvas = el.querySelector('cropper-canvas')!.getBoundingClientRect()
      const image = el.querySelector('cropper-image')!.getBoundingClientRect()
      const crop = el.querySelector('cropper-selection')!.getBoundingClientRect()
      return Math.abs(image.height - canvas.height) < 1
        && Math.abs(crop.top - image.top) < 1 && Math.abs(crop.bottom - image.bottom) < 1
        && Math.abs(crop.width / image.width - 3003 / 4209) < 0.001
    }), `Initial crop geometry with ${reducedMotion}`).toBe(true)
    ...
    if (reducedMotion === 'no-preference') await page.keyboard.press('Escape')
  }
