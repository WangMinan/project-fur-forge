-- T51-F7：允许新的 site-display-v2 无水印公开衍生身份，同时保留 v1 行。
-- SQLite 不能原地修改 CHECK；只重建 asset_variants，既有对象与行原样复制。
...
CREATE TABLE `__new_asset_variants` (
	...
	CONSTRAINT "asset_variants_public_protection" CHECK(`storage_scope` != 'PUBLIC' OR `protection_mode` = 'watermark' OR `recipe_version` IN ('site-display-v1', 'site-display-v2', 'return-display-v1', 'contact-qr-v1')),
...
WHEN
  ...
  OR NOT (
    ...
    OR (NEW.`media_role` = 'return_photo' AND NEW.`usage` IN ('preprocess', 'return-wall'))
...
WHEN
  ...
  OR (
    ...
    AND (NEW.`width` > 4096 OR NEW.`height` > 4096 OR (NEW.`status` = 'READY' AND NEW.`byte_size` > 20000000))
  )
  OR (
    NEW.`source_variant_id` IS NULL
    AND NEW.`input_sha256` != (SELECT `sha256` FROM `assets` WHERE `id` = NEW.`asset_id`)
  )
  OR (
