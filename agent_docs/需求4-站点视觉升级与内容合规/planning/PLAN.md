...
- 聚合最多投影按现有 comparator 得到的前三项 available。
- `HomeCurrentAdoptions` 使用一个主角色牌和最多三个真实角色索引，不恢复旧双列卡片。
- 没有 available 时隐藏；adopted 仍可在精选中出现。
- core 覆盖一项、两项、三项上限和 adopted 排除；smoke 只证明首页/目录入口可达，不断言审美细节。
...
- T45/V11 领养：一次媒体揭示和 caption 交接，保留目录与当前角色两个行动并保持一屏完整表达；V11 将旧单项基线升级为最多三个真实角色索引，主媒体使用同款聚焦，删除到 Footer 的额外尾部 padding。
...
- T46-F8：代表作品标题复用三业务幕左上标题 CSS；桌面双图在保留底部留白的前提下放大并增加间距，说明文案精简。取消代表作品竖图到详情舞台的共享形变，只保留入口 query 与直接路由切换；委托/领养共享对象不受影响。
