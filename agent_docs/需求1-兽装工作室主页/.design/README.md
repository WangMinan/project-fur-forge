...
- v5 中的占位名称、隐藏价格、20 MB 上传提示、旧辅助色和狗头闪电图形不属于上述锁定范围；2026-07-29 的 SPEC/PLAN 与景宸例图证据覆盖这些历史内容。
...
- `../materials/景宸品牌例图_2026-07-29/`：正式中文名、临时英文名、单件全装掉落事实、Logo/QQ 二维码禁用边界和可复现蓝色聚类；只把证据化色阶与业务事实带入生产设计，不把宣传海报构图直接复制为网页。
