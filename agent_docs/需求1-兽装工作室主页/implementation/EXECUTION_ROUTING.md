...
> **生效日期**：2026-08-04（T23–T25 实现、自动化自检和用户人工核验完成，等待独立 Review）。
...
| `ACCEPTANCE` | 业务和视觉结果最终确认 | 用户；T21、T22、T23–T25 已确认，后续按任务或门禁验收 |
...
- T01–T22、GATE-06、GATE-07、EXT-01 与 EXT-02 已完成；T22 独立 Review 和用户人工验收均已收口。
- T23–T25 实现、自动化自检和用户 2026-08-04 人工核验已完成；`/works` 出厂照边界和 `recipe-v2` 设定图左右双水印已通过正式素材真实 OSS 与公开浏览器回归。
- T23、T24、T25 均未勾选；T29 重定向和 T37 展会矩阵未提前进入。
...
### C2 · T23–T25 角色化媒体与常规领养

- T23（工程、自动化和用户人工核验完成，待 Review）：设定图/出厂照关系、数量、顺序、主图、完整画布、按需 recipe 和活动水印约束已接入；
- T24（实现、自动化和用户人工核验完成，任务未勾选）：管理端角色化快速编辑、真实同源原图预览、活动水印预览、上传恢复和列表媒体摘要已接入；
- T25（实现、自动化和用户人工核验完成，任务未勾选）：regular adoption 发布、`/adoptions` 横版设定图页面、统一详情媒体分区、`/works` 出厂照边界和 `recipe-v2` 左右双水印已接入；
- 当前等待新的 GPT-5.6 Sol 上下文独立 Review，并在该上下文复核正式素材管理全链；后续仍直接在 `main` 上串行。

### C3 · T26–T30 完整公开站

- T26/T27：后端先锁定营业状态和站点内容契约，前端实现委托、关于、约定、联系；
- T28/T29：前端完成首页内容顺序、作品筛选、详情导航，后端补齐公开投影与重定向；
