...
- 普通页面不使用 scroll-jacking；首页按用户 2026-08-21 新确认，仅在 `min-width: 1024px` 提供逐幕 wheel 导航（Hero → 代表作品 lead → 继续浏览 → 自设委托 → 设定领养 → Footer）。1023px 及以下继续原生纵向滚动，不拦截 touch；仍不使用长时间 pinned scroll、强制横向叙事、持续视差、全屏粒子或背景音效。
...
- 首页与委托页都通过横/竖二级 Tab 只显示当前方向；首页按方向编辑多项并显示 `横版 X/5 · 竖版 Y/5`，委托显示 `横版 X/1 · 竖版 Y/1`。
- 提供桌面/手机画框预览、画面内可拖焦点和水平/垂直焦点滑杆；不新增 pair/crop/focal 表。
