...
> **写入日期**：2026-07-28。
> **当前状态**：阶段 3 TASKS 已初始化；尚未进入阶段 4，所有实施任务均未开始。
...
- [ ] **GATE-06 · 阶段 4 实施授权**：只有用户明确要求“进入阶段 4”或“从 T01 开始实施”后，才记录授权日期与原话并勾选；未勾选时禁止开始 T01–T53。
...
- [ ] **T03 · 建立共享契约与错误语义**：建立 Zod 枚举、请求/响应 Schema、公开 DTO、管理员 DTO、资源版本/幂等键和统一错误格式；数据库行对象禁止直接跨到客户端。_创建：`shared/schemas`、`shared/types`、服务端 mapper；依赖：T01；验证：类型检查、Schema 单测和“公开 DTO 不含价格/联系方式/付款备注/原图键/认证字段”泄漏守卫。_
- [ ] **T04 · 实现公开站视觉系统与导航壳**：把 `../.design/public-site/DESIGN_TOKENS.md` 翻译为实际 CSS/Tailwind Token，创建 `PublicHeader`、`PublicMobileNav`、`PageIntro`、`ResponsiveAsset`、`PublicFooter`；首页覆盖态与内页白底态、390/768/1440 响应式、键盘焦点和减少动效同时完成。_创建：公开基础组件；依赖：T01–T03；验证：三视口截图、公开站无后台入口/内部文案/横向溢出，公开组件 Bundle 不包含后台组件。_
...
- [ ] **T06 · 用类型化夹具完成作品列表与详情视觉样张**：实现用途 × 装型交集筛选、结果数、人工顺序、等大 3:4 网格、空结果和 `/works/{slug}` 详情图集；详情是独立路由，灯箱只服务图集。_创建：`WorkFilterBar`、`WorkGrid`、`WorkCard`、`WorkDetailGallery`；依赖：T04；验证：1440 × 900 四列、768 × 1024 三列、390 × 844 两列，筛选不改变人工顺序，普通链接在关闭 JavaScript时仍进入详情。_
...
- [ ] **T10 · 建立作品、业务状态与稳定 URL Schema**：实现 `works`、`slug_redirects`、`events`、`business_statuses` 及必要约束；一件作品对应一个角色，领养方式、六种业务状态和发布状态分离，`展会出售中` 约束当前展会，首次发布后 slug 默认冻结；作品默认顺序、角色领养列表顺序和首页精选顺序分别持久化。人民币金额使用明确币种/最小货币单位字段，美元字段保留但一期禁用，未来启用不改变作品 ID 或关联。_创建：核心 Schema/迁移/夹具；依赖：T09；验证：唯一/检查/事务/并发版本、三类人工顺序独立性、人民币/美元独立字段与禁用美元夹具，普通作品可无领养字段，同一角色不产生重复作品。_
...
- [ ] **T12 · 实现领域服务、公开投影与夹具闭环**：为作品、媒体、站点内容和统计建立服务层与显式 mapper；提供虚构开发夹具和最小公开投影，不把 Vue 页面直接耦合到 Drizzle。_创建：服务/Repository 边界、fixture loader；依赖：T10–T11；验证：夹具可重复装载不重复，公开投影泄漏测试、事务回滚和 SSR 请求间无模块级状态泄漏。_
...
- [ ] **T17 · 执行 OSS 预检、授权配置与复验门禁**：第一步只读验证杭州 Bucket/region/上传 Endpoint、两个完整开发 origin、CORS 方法/请求头、账号级与 Bucket 级 Block Public Access、已发布衍生图 `public-read` 能力、原图 `private` 和 `test/<run-id>/` 前缀隔离；报告逐项记录期望值、实测值和 PASS/FAIL。任一 FAIL 均保持 T17 未完成并停止：账号级变更与上传 CNAME 只由用户在控制台处理；Bucket CORS/BPA 等变更必须由用户另行明确授权精确目标后执行或由用户手工完成，随后重新运行只读复验。_创建：preflight/config 命令与脱敏记录模板；依赖：T16；验证：全部项目 PASS 后才能勾选 T17 和进入 T18；不上传生产素材、不静默修改任何设置、不以 Nitro 代理或双 Bucket 绕过。_
- [ ] **T18 · 实现 V4 条件直传会话与浏览器上传**：浏览器校验格式/字节/数量/像素并计算 SHA-256、Content-MD5；服务端创建 5 分钟会话和不可预测私有 Key，签名固定 `Content-Type`、MD5、SHA-256 metadata 与禁止覆盖；客户端直接 PUT 并显示进度。_创建：presign API、上传 composable、`MediaUploader`/`UploadItem`；依赖：T17；验证：20,000,000 字节边界、错误格式/数量、请求头篡改、重复 PUT/Key 覆盖拒绝，图片二进制不经过 Nitro。_
...
- [ ] **T26 · 完成作品草稿 CRUD 与四步新建**：将 T07 的四步样张接入真实 Schema/API/SQLite/OSS；按用途动态要求出厂照或领养设定图，每步可保存草稿并恢复，页面显示真实服务端结果。_修改：`WorkCreateStepper`、作品 API；依赖：T12、T16、T25、T08；验证：普通/领养/展示作品三条创建路径，重复提交不产生重复作品/媒体，关闭后可恢复已保存草稿。_
- [ ] **T27 · 完成作品快速编辑、预览、发布、下架与显式改址**：已有作品直接编辑文字、业务/发布状态、当前展会、领养资料、价格、图片与 SEO；发布检查执行最小矩阵，slug 默认只读，显式纠错写 redirect；预览读取草稿和私有衍生图。_修改：`WorkEditor`、`PublicationChecklist`、预览/发布 API；依赖：T22、T26；验证：所有发布阻断、展会规则、完成交付重开确认、旧 slug 301、下架 ACL/公开投影、并发版本冲突。_
...
- [ ] **T30 · 完成领养方式、六种状态与当前展会管理**：实现常规领养/展会掉落切换、六种状态回退、当前展会维护和“已完成交付”重开确认；系统不增加制作完成度或展会历史。_创建/修改：`EventEditor`、作品业务状态编辑；依赖：T27；验证：展会出售中一致性、切换不要求头部实拍、公开字段只含展会名称。_
...
- [ ] **T32 · 完成首页、关于/约定、联系与站点 SEO 内容管理**：按明确对象维护首页短文/入口、工作室事实、七类基本约定、官方邮箱/QQ/抖音、防诈骗和默认 SEO；使用受限字段/Markdown，不允许任意脚本或未清洗 HTML。_创建：`SiteContentEditor` 分页与 API；依赖：T12、T16、T08；验证：无万能页面树、无公告/联系消息箱、公开内容不包含占位/内部说明，敏感字段不可写入公开区。_
...
- [ ] **T34 · 完成 UTF-8 BOM CSV 与原图复选导出中心**：分别导出业务数据和图片清单的 SPEC 最低字段，排除二进制、凭据、Object Key、校验值和结构版本；原图仍走短时授权下载。_创建：`ExportPanel`、流式 CSV API；依赖：T12、T16、T24、T30；验证：Excel/文本打开中文正常、字段/转义/公式注入防护、公开客户端拒绝、导出内容泄漏守卫。_
...
- [ ] **T36 · 接通作品列表、筛选与 canonical 详情**：把 T06 样张连接公开投影；SSR 输出人工顺序网格和 `/works/{slug}` 详情，客户端筛选取交集并显示数量，保留未来分页边界。_修改：T06 组件与作品 SSR 服务；依赖：T27–T28、T08；验证：普通/领养/展示作品、空结果、无 JavaScript、错误/下架 404、公开 DTO 无敏感字段。_
...
- [ ] **T38 · 完成角色领养 SSR 列表**：展示领养营业状态、水印设定图/作品图、方式、六种状态和适用展会，并读取独立的领养列表人工顺序；卡片使用普通链接进入 `/works/{slug}`，不增加筛选、价格、登记、支付或第二详情正文。_创建/修改：`AdoptionGrid`/`AdoptionCard`；依赖：T27–T31、T08；验证：常规/掉落/已交付、无头部实拍仍可展示，领养顺序不受作品展示默认顺序影响，公开投影无领养人敏感资料。_
...
- [ ] **T42 · 完成页面级 Meta、canonical、分享信息与重定向**：首页、所有一级页和作品详情输出独立 title/description/canonical/Open Graph；`/adoptions/{slug}`、`/terms`、旧作品 slug 永久重定向且无重复正文；正式名称可由站点设置统一替换。_修改：页面 SEO composable、服务端 redirect；依赖：T35–T40；验证：状态码、Location、canonical、内部链接、分享图只使用公开衍生图。_
- [ ] **T43 · 完成 Sitemap、robots、结构化数据与无 JavaScript 门禁**：Sitemap 从 SQLite 公开投影生成页面、作品和图片项；后台/预览/API/dev noindex；只实现有真实依据的 Organization、BreadcrumbList 和作品展示实体。_创建：SEO modules/data sources；依赖：T42；验证：关闭 JavaScript仍有标题/正文/链接/alt，兼容路径和下架内容不进 Sitemap，无虚构评价/价格/库存。_
...
- [ ] **T53 · 完成一期验收映射与阶段 5 交接**：逐项核对 SPEC 一期验收和待真实环境验证，链接测试、截图、OSS、恢复、资源、内容确认、访客任务与景宸操作记录；T01–T52 必须全部完成。若任务无法完成则保持未勾选并登记阻断；任何范围放弃都必须有用户带日期的明确批准，影响 SPEC/PLAN 时先返回对应阶段变更契约，不能在 T53 静默豁免。_修改：TASKS、ARTIFACTS、STATE、REVIEW；依赖：T01–T52；验证：SPEC ↔ PLAN ↔ TASKS ↔ 代码 Reader Test 的 blocker=0、P1=0；P2/P3 逐项记录处置与一期影响，阶段 4 前不得提前勾选。_
...
| 图片限制、原图、EXIF、裁切、水印、衍生组 | T11、T17–T25、T50 | 图片夹具、OSS 契约、匿名访问测试 |
...
| 公开不显示价格/敏感字段，未来字段可演进 | T03、T10–T12、T27、T36–T40 | 类型/Schema/泄漏守卫 |
...
- **FUTURE-01**：在线估价、站内表单、登记、账号、订单、支付、多语言、公开价格、公告、完整无障碍专项、暗色主题、AVIF、GSAP、自动媒体队列、PostgreSQL/独立 API。

## 闭环结论

- 当前只完成阶段 3 的任务拆解与设计输入；T01–T53 均未实施。
- 下一步只有在用户明确授权进入阶段 4 并勾选 GATE-06 后，才从 T01 开始，并在 T08 设置第一次生产视觉确认点。
- 正式素材、工作室名称、基本约定法律/适用性审阅和真实设备可达性属于最终验收门禁；缺失时相关任务保持未勾选，不以占位内容代替。
