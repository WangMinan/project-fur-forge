...
所有代码、文档、Review 和修复必须通过独立分支与 PR 合入 `main`：

1. 写入前读取远端最新 `main`，并从 `origin/main` 新建任务分支；功能使用 `feat/*`，修复使用 `fix/*`，纯文档使用 `docs/*`；
2. 禁止直接在 `main` 提交或推送；任务分支提交并推送后，发起以 `main` 为目标的 PR；
3. PR 的相关 GitHub Actions 通过并完成必要 Review 后才能合并；不得把本地通过描述为远端全绿；
4. 后端 → 前端 → Review → 用户验收依次交接；
5. 提交保持小而可回滚，不 force push、不硬 reset、不重写已验收历史；
6. 只读分析可以并行，任何写入必须串行。
...
- 所有应用源码、迁移、运行时 Schema、环境模板、OSS/ESA 核心实现、正式部署基线、自动测试、CI 和独立 Review 必须在阶段 E 完成，并由 GATE-E 冻结唯一上线 SHA/镜像；
...
低分辨率设定图同样允许上传和保存，不用笼统尺寸错误阻断发布。发布时自动使用内嵌 FFmpeg Lanczos 生成保持比例的私有适配源，管理端持续提示放大不恢复细节且完整原图保留。
...
- 正式域名、ESA 边缘/源站配置、宿主机 HTTP-only Nginx、app-only Compose、空卷、升级、回滚和恢复所需代码/模板/命令由 T52-E1～E6 开发；真实远程执行由 T53-F1～F5 完成。
