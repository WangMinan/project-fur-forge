...
1. `studio_photo` 继续走现有条件直传、作品关系、publication operation 和 `recipe-v2` 水印链；上传、保存和发布检查都不再因为原图像素不足而硬阻断；
...
`T46 → T51 → T51-F1 → T51-F2 → T52-E1～E6 → T51-F3 → T49 → T50 → GATE-E → T53-F1～F5`
