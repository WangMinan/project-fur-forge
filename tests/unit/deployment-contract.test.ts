...
describe('T52-E6 production deployment contract', () => {
  ...
  it('keeps publication manual and exercises the target deployment shape in CI', () => {
    ...
    const releaseTests = source('scripts/run-release-tests.mjs')
    const preflight = source('scripts/oss-preflight.mjs')
    ...
    expect(quality).toContain('RELEASE_PRODUCTION_VERIFIED_BY_IMAGE: "1"')
    expect(quality).toContain('google-chrome --version')
    expect(quality).not.toContain('playwright install --with-deps chromium')
    expect(quality).toContain('data-testid="public-home"')
    expect(quality).toContain('data-testid="admin-login"')
    expect(releaseTests).toContain("process.env.RELEASE_PRODUCTION_VERIFIED_BY_IMAGE !== '1'")
    expect(preflight).not.toContain('oss-write-prefix-overreach-denied')
    expect(preflight).not.toContain('outside-allowed-prefix')
