...
2. 低分辨率设定图继续走现有条件直传、作品关系和 publication operation，不新增上传器或第二套状态机；发布检查返回非阻断适配提示，operation 在 `PREPARING_SOURCE` 阶段按当前用途计算最小几何尺寸，用内嵌 FFmpeg Lanczos 生成保持比例的私有处理源，再进入当前 `recipe-v3` 水印链；
...
### 3.2.3 T51-F3 低分辨率出厂照适配

用户在作品编辑器上传出厂照时补充以下修复：

1. `studio_photo` 继续走现有条件直传、作品关系、publication operation 和当前 `recipe-v3` 水印链；上传、保存和发布检查都不再因为原图像素不足而硬阻断；
2. 发布检查返回非阻断 `studioPhotoNeedsPreprocess` 提示；operation 在 `PREPARING_SOURCE` 阶段按本次 `detail` / `work-card` 用途计算最小几何尺寸，用内嵌 FFmpeg Lanczos 生成保持原比例的私有适配源；
3. 使用独立不可变 `studio-photo-upscale-lanczos-v1` 身份，永久原图不覆盖；公开生成和 3:4 卡片裁切使用适配源实际尺寸；
4. 适配失败保存稳定失败码，管理端说明原图已保留并允许重新发布重试；同步共享 Schema、管理提示、FFmpeg/recipe、publication integration 与真实浏览器测试。

2026-08-10 已按上述方案完成工程实现和相关本地门禁；用户确认当前浏览器行为可用，T49 独立 Review 仍保持开放。

### 3.2.4 T51-F4 管理端 FFmpeg 进度与作品竖图水印

用户在进入 T49 前补充两项阶段 E 修复：

1. 抽出复用的管理端 FFmpeg 等待进度组件，覆盖大文件上传私有预处理、Hero 放大、设定图/出厂照发布适配及处理重试；显示当前阶段和已等待时间，视觉层级与 OSS 上传进度一致；
2. 单张静态图没有可信的连续帧百分比，因此 FFmpeg 阶段使用动态不定量进度，不能用估算值冒充真实百分比；OSS 字节上传和公开变体计数仍展示各自真实进度；
3. 把内嵌 FFmpeg 从同步子进程改为异步子进程，保留固定 binary、超时、输出上限和安全错误，确保处理期间 Nitro 仍可响应轮询与其他管理请求；
4. 作品水印升级为 `recipe-v3`：3:4 `work-card` 按 480 px 最小档位、竖版 `studio_photo` `detail` 按 960 px 最小档位随输出宽度等比放大，仍各自只有一个居中水印；横版 `detail` 尺寸与设定图左右双水印保持不变；
5. 新像素生成新不可变 Key；公开投影优先完整 `recipe-v3`，整体回退完整 `recipe-v2` / `recipe-v1`，不混搭；补充 FFmpeg 异步执行、配方身份/处理串、管理可见反馈和浏览器回归。

### 3.2.5 T51-F5 竖版详情水印与卡片/预览对齐

用户在真实作品中确认 3:4 作品卡已经使用预期的大水印，但同一张竖图的公开详情与后台公开水印预览仍按 960 px 参考宽度生成，导致相对尺寸只有卡片的一半。修复方案为：

1. 竖版 `studio_photo` `detail` 与 3:4 `work-card` 统一使用 480 px 水印参考宽度；两种用途仍各自只生成一个居中水印；
2. 后台公开水印预览继续调用同一 `buildWatermarkProcess`，不增加 CSS 叠层或第二套预览配方；
3. 参考宽度进入现有配方身份，因此详情像素变化生成新的 `recipe-v3` 不可变 Key，不复用旧小水印对象；
4. 横版 `detail`、设定图左右双水印、活动 profile 参数和旧配方整体回退保持不变；
5. 补充处理串、不可变身份、真实作品预览与详情/卡片视觉一致性回归。

### 3.2.6 T51-F6 公共页脚备案居中与三栏重排

用户在正式 ICP 值写入后发现宽屏页脚左右留白失衡，且备案号混在右侧第三行。修复方案为：

1. 保持左侧完整品牌、中间公开导航、右侧法务的三块结构，以明确且对称的列间距取代由右侧内容宽度挤压出的不均衡空档；
2. 把 ICP 与后续公安联网备案移动到中间独立备案行，导航在上、备案在下；未取得公安备案号时不输出占位文案，继续复用既有严格配置和空值隐藏；
3. 把开源软件声明移动到右侧法务链接行，使右侧在足够宽的桌面视口固定为版权声明与法务/设计署名两行；
4. 保持管理端登录页 `brandOnly` 行为，并在窄屏允许安全换行，不引入横向滚动。

### 3.2.7 T51-F7 管理私有预览与首页 Hero 4K 配方

用户在正式素材与 OSS/ESA 流量观察中确认私有端重复读取完整原图、公开首页高分屏只得到 1920 px Hero。修复方案为：

1. 管理卡片统一请求 `w=320`，编辑器较大预览统一请求 `w=640`；永久原图只通过明确的 `original=1` 查看/下载动作读取，继续 `no-store`，缩略失败不回退原图；
2. 站点配方升级为 `site-display-v2`，首页横版增加 2880×1620 与 3840×2160，Hero WebP 质量固定为 90，站点入口质量不变；
3. 公开投影优先完整 v2，在旧图升级完成前整体回退完整 v1；新路径不覆盖旧对象，响应式 SourceSet 继续按视口和 DPR 选择；
4. 增加默认 dry-run、可重复执行的 `pnpm media:upgrade-site-display-v2`，并在发布镜像的 `ops.mjs` 暴露同一参数契约，远端通过一次性 app 容器执行；
5. 补充配方、迁移、旧图重建幂等、CLI 参数透传、管理端 URL 和部署文档测试。

### 3.2.8 T51-F8 全局水印流程与首页精选排序优化

用户在空库启用水印和首页精选维护中确认当前交互暴露了不必要的真实预览门槛与内部排序编号。修复方案为：

1. `/admin/site/branding` 删除真实预览组件和预览门槛，收敛为 Logo 候选、参数、影响摘要与一次确认刷新；新增组合 `PUT /api/admin/v1/site/branding/watermark`，在一个事务入口中创建/复用 profile 并启动既有持久重建 operation；
2. 空库目标数为 0 时允许激活 profile；已有公开作品仍先生成/校验、后原子切换，保留切换前失败、首次失败和切换后清理失败的既有恢复语义；旧预览 API 只保留兼容，作品端真实预览继续使用活动 profile 且不作为发布门禁；
3. `/admin/works?tab=featured` 按首页顺序显示全部精选，提供置顶、上移、下移、置底和移出精选；完整 `{id, expectedVersion}` 有序集合在单个事务中校验唯一性、当前精选集合和版本，冲突统一返回 `FEATURED_ORDER_CONFLICT`；
4. 移除列表和编辑器的裸数字排序输入；新增精选自动追加、取消自动压紧，下一次写入统一修复历史重复/空洞序号；数据库继续保存连续 `0..n-1`，不增加迁移、唯一索引或拖拽依赖；
5. 共享首页精选上限从 6 改为 12，公开只取排序最前的 12 件已发布作品；保留横向轨道、卡片尺寸和图片懒加载，并补第 7～12 件可访问、边界、键盘、焦点、减少动效和三视口回归。

### 3.2.9 T51-F9 首页与管理端动效、移动导航

用户确认把精选排序的清楚位移反馈扩展到首页/委托大图，并在不抢夺图片主体的前提下增强公开首页动效与窄屏管理导航。实施方案为：

1. `/admin/site/home` 的首页和委托 placement 都改用稳定 ID 的 `TransitionGroup`；草稿卡使用独立稳定 key，只增加 FLIP move class，不改变完整顺序提交、资源版本、启停或保存协议；placement Tab 和 keyed 内容面板增加轻量颜色、阴影、淡入位移；
2. 抽出单一 `ADMIN_NAV_ITEMS`，桌面横向导航、侧栏和新的 `AdminMobileNav` 共用八个入口；`<1024px` 使用三横线按钮和全屏抽屉，`1024–1279px` 保留横向导航，`>=1280px` 保留侧栏，窄屏账号/退出放入抽屉底部；
3. 抽出公开/管理全屏导航共用的焦点陷阱、Escape、背景 `inert`、滚动锁定、路由关闭和焦点归还逻辑；公开与管理抽屉各自只负责内容与样式，面板和菜单项使用克制位移/错峰；
4. Hero 文案与控件在首次加载时依次淡入上移；精选、委托投递、当前领养和最新动态通过小型一次性视口包装组件首次入屏，SSR/无 JavaScript 默认可见且返回滚动不重播；
5. 仅给作品、领养和业务入口等可点击卡片增加精细指针抬升与图片层次；动态卡片不增加误导性 hover；全部新增动效只使用 `transform`、`opacity`、颜色和阴影，减少动效时关闭位移、错峰和排序过渡；
6. 补首页/委托真实重排、管理抽屉边界与无障碍、Hero/入屏/hover/公开抽屉/reduced-motion、无 JavaScript内容完整性自动化，并以双 Host、三固定视口及 1023/1024 px 边界做浏览器复验。

本任务不新增数据库、迁移、HTTP API、排序协议、连续动效或前端依赖。
...
T49 在 T46、T51/T51-F1～F6、T52-E1～E6 全部完成后执行。以同一最新 SHA 取得 frozen install、lint、typecheck、unit、integration、build、production verify、secret scan、Compose、image-build 和 E2E 完整结果。
...
`T46 → T51 → T51-F1 → T51-F2 → T52-E1～E6 → T51-F3 → T51-F4 → T51-F5 → T51-F6 → T51-F7 → T51-F8 → T51-F9 → T49-R1 → T50 → GATE-E → T53-F1～F5`
