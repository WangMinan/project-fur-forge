...
> **状态**：`GATE-01`、T01～T16、预验收修正 T15-F1～T15-F4、T17-F1～T17-F4 已关闭。T16-R1 新 SHA 独立复查和 T17 最终用户签署保持开放。
...
- [x] **T17-F3 · 第三轮用户反馈修复**：删除作品目录访客总数；把自设委托与掉落领养展开为一级导航；压缩领养卡片元信息并验证跨行间距；以访客为对象重写仍为默认值的委托、关于、条款、隐私和防诈骗文案，新增保留管理员编辑的条件式前向迁移；同步定向测试、三视口、lint/typecheck/build 和实施记录。 _迁移 integration 17/17、公开 E2E 32/32、后台定向 E2E 1/1、unit 179/179、lint/typecheck/build/verify 和实际 Chrome 三视口通过；记录：[`notes/T17-F3-NAV-ADOPTION-COPY-2026-08-14.md`](./notes/T17-F3-NAV-ADOPTION-COPY-2026-08-14.md)。不代签 T16-R1 或 T17。_
- [x] **T17-F4 · 目录节奏与领养命名统一**：把公开导航及 `/adoptions` H1/SEO/分页名称统一为“设定领养”；让 `/works` 工具栏到首图的间距复用 `/adoptions`、`/returns` 既有 `var(--space-6)`，并用跨页几何 E2E 与 Chrome 三视口锁定 32 px。 _相关公开 E2E 37/37、unit 179/179、integration 190/190、lint/typecheck/build/verify 通过；同时修复 Actions run `31719392357` 暴露的三处迁移版本过期夹具。记录：[`notes/T17-F4-CATALOG-RHYTHM-NAMING-2026-08-14.md`](./notes/T17-F4-CATALOG-RHYTHM-NAMING-2026-08-14.md)。不代签 T16-R1 或 T17。_
- [ ] **T16-R1 · 新 SHA 独立复查**：由未参与 T17-F1～F3 实现的新上下文核对动画按需加载/reduced-motion、动态筛选与卡片复用、目录与领养卡片三视口、文案迁移保留边界和文档一致性；旧 T16 不代签新 SHA。
- [ ] **T17 · 用户验收与闭环**：用户确认导航、联系方式、模板、搜索和动态发布体验；更新 STATE、ARTIFACTS、models、REVIEW 与任务勾选。 _文档闭环；依赖：T16。_

## 闭环结论

- `GATE-01`、T01～T16、T15-F1～T15-F4、T17-F1～T17-F4 已关闭；T16-R1 与 T17 保持开放。
- 需求2的等价实现补丁当前已包含在 `main`，旧功能分支/PR 状态不再是当前门禁；T17 用户验收保持未勾选，不能由工程 Review 或 CI 代签，旧 T16 也不能代签 T17-F1～F4 的后续代码。
