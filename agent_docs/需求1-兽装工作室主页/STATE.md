...
> 下一步是三项的新上下文独立 Review 与用户人工验收。**

已落地事实（2026-08-08）：

- 返图一图一记录模型、`return_photo` 私有上传、`return-display-v1`
  无水印公开衍生、发布/下架 operation 与恢复；
- 管理端返图管理（列表 + 一图一记录编辑）与公开一级导航返图墙 `/returns`；
- 轻量展会掉落：四选项业务类型、`event_name` / `event_time`、
  `/adoptions` 三筛选、卡片与详情展示；
- 前向迁移 0022（返图）与 0023（展会字段）；
- 本地门禁：lint、typecheck、unit 122/122、integration 152/152 通过。
...
| 任务 | 当前状态 | 下一步 |
| --- | --- | --- |
| T35 | **工程完成，待独立 Review** | 新上下文 Review 一图一记录、关联、版本与隐私 |
| T36 | **工程主体完成，待补验证与 Review** | 390×844 手机闭环、返图 SIGKILL/重复重启重放、独立 Review 与用户验收 |
...
5. 补齐 T36、T37 的 390×844 手机闭环与返图 SIGKILL / 重复重启重放；
6. 新上下文独立 Review T35、T36、T37（同一实现者不得代签）；
7. 用户对 T36、T37 执行浏览器人工验收；
8. T42 阶段 D 总门禁（由用户勾选）；
