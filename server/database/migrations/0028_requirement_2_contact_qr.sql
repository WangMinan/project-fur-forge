-- T03 联系二维码媒体：扩展 site/contact 上传归属、contact_qr 资产角色，
-- 以及 contact-qr-v1 公开无水印 PNG 派生约束。
-- SQLite 不能直接修改 CHECK，因此只新增前向迁移并重建三张受影响的表。
...
CREATE TABLE `__new_asset_variants` (
	...
	CONSTRAINT "asset_variants_media_role" CHECK(`media_role` IN ('design_sheet', 'studio_photo', 'home_hero_landscape', 'home_hero_portrait', 'return_photo', 'contact_qr')),
	CONSTRAINT "asset_variants_usage" CHECK(`usage` IN ('preprocess', 'work-card', 'detail', 'design-sheet', 'home-hero-landscape', 'home-hero-portrait', 'commission-hero-landscape', 'commission-hero-portrait', 'home-entry-commission', 'home-entry-adoption', 'return-wall', 'contact-qr')),
	...
	CONSTRAINT "asset_variants_public_protection" CHECK(`storage_scope` != 'PUBLIC' OR `protection_mode` = 'watermark' OR `recipe_version` IN ('site-display-v1', 'return-display-v1', 'contact-qr-v1')),
...
CREATE TABLE `__new_assets` (
	...
	CONSTRAINT "assets_contact_qr_png" CHECK(`role` != 'contact_qr' OR (`mime_type` = 'image/png' AND `byte_size` <= 20000000 AND `width` = `height` AND `width` >= 320 AND `fit_mode` = 'contain')),
...
CREATE TABLE `__new_upload_sessions` (
	...
	CONSTRAINT "upload_sessions_contact_qr_png" CHECK(`media_role` != 'contact_qr' OR (`expected_content_type` = 'image/png' AND `expected_bytes` <= 20000000 AND `expected_width` = `expected_height` AND `expected_width` >= 320)),
...
WHEN
  ...
  OR NOT (
    ...
    OR (NEW.`media_role` = 'contact_qr'
      AND NEW.`usage` IN ('preprocess', 'contact-qr'))
  )
BEGIN
...
WHEN NEW.`usage` = 'preprocess' AND NEW.`status` = 'READY'
...
WHEN NOT EXISTS (
  SELECT 1 FROM `assets` WHERE `id` = NEW.`source_asset_id`
    AND `role` = 'watermark_logo' AND `status` = 'READY'
    AND `mime_type` = 'image/png' AND `sha256` = NEW.`logo_digest`
)
BEGIN
...
WHEN NEW.`asset_id` IS NOT NULL AND NOT EXISTS (
  SELECT 1 FROM `assets` WHERE `id` = NEW.`asset_id`
    AND `role` = 'return_photo' AND `status` = 'READY'
)
BEGIN
