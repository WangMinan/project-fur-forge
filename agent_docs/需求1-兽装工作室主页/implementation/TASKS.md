...
>
> T46 访问统计 → T51/T51-F1～F6 公开体验与媒体修复 → T52-E1～E6 生产媒体与部署能力 → T49 同一 SHA CI/独立 Review → T50 最终受控环境回归 → GATE-E。
...
- [ ] 新上下文独立 Review preflight、Nitro 产物、Docker 守卫及停止/恢复
  边界，并覆盖本次 CORS/旧对象判定调整、T51-F5 水印对齐、T51-F6 页脚布局和 T51-F7 媒体升级；实现者不得代签。
- [ ] Review 通过后重新发布不可变镜像，保存新的
  `repository@sha256:digest`，远端不得继续使用旧镜像。
_依赖：包含 T51-F7 的最新实现 SHA；完成后才可继续 T50/GATE-E 或远端 live preflight。_
