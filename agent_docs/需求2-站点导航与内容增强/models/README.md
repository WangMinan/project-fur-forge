...
> **状态**：T01～T16、T15-F1～T15-F4 与 T17-F1～T17-F4 已完成，其等价实现补丁当前已包含在 `main`。T17-F3 不增加表、列、媒体投影或公开 DTO，只增加一条条件式默认文案前向迁移；T17-F4 也不改变持久模型。下一步仍保留 T16-R1 新 SHA 独立复查与 T17 用户验收。
