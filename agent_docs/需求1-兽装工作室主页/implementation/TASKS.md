...
> **角色**：PLAN 的唯一可勾选实施分解。
> **状态**：T01–T25、GATE-06、GATE-07、EXT-01 与 EXT-02 已完成。T23–T25 已收口。T26–T27 服务端、首版前端与 2026-08-04 用户追加视觉/管理体验跟进已完成实现方复核；**OQ-120 已有候选稿但仍开放**，独立 Review 与用户验收前两项任务保持未勾选。任务编号保留 T01–T53。
...
**OQ-120**（T26–T27 真实文案 10 项）已有一版原创候选稿，仍需用户逐项确认并阻断页面最终收口；见 `requirements/SPEC.md` §6.7。候选稿不得自动写入数据库。最终小图标与部署参数分别在 T30、T34/T52 前确认。
