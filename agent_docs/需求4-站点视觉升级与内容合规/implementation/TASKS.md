...
- [x] **T47-F6 · 委托首屏视觉动线收紧**：首页自设委托幕与 `/commission` 删除“从角色设定出发”且不补替代 slogan；`/commission` 在 `1023px` 以下保留竖版素材但改为不高于 `50svh` 的 `4:5` 编辑式裁切，使标题、营业状态、说明与主行动进入目标手机/平板第一视口；`1024px` 及以上继续使用左叙事、右横图分栏。未修改媒体模型、后台横竖集合、业务文案数据、Footer 或路由。Handoff：`implementation/notes/2026-08-29-T47-F6-COMMISSION-LEAD-FLOW.md`。
