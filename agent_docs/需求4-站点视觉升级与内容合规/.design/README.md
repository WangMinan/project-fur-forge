...
- 只保留一个停靠幕，最多两件代表作品；桌面左侧并排两张较小的 3:4 竖图，右侧承担说明和行动。
...
- 桌面图片高度必须为右侧按钮和留白让位，标题、说明、按钮与双图同屏；移动端文字在上、双图在下，不依赖横向滑动。
...
- 代表作品左侧双图与右侧标题、说明、唯一目录按钮从固定 Header 下方开始，并在同一动态视口内完成；没有轨道按钮。
