...
- 公开页面不得用 `x-oss-process` 临时加工，也不得用 CSS 叠 Logo 冒充发布结果；管理端允许服务端固定尺寸的 OSS 私有缩略图，由认证同源接口返回字节，处理参数不得由客户端自由指定；
...
| 管理端原图/Logo/处理源 | 私有对象 | 登录后同源 GET | 卡片 `w=320`、出厂照 `w=640`、Hero 编辑/作品设定图/领养封面/委托详情 `w=1280`、显式原图内联预览、`no-store` |
...
- 两种竖图用途统一以 480 px 为水印参考宽度，随 480/768/1200 或 960/1600/2400 输出档位等比放大；详情图、3:4 卡片与后台公开水印预览因此保持相同的相对尺寸；
...
管理端私有预览不属于公开投影：列表/卡片只请求 `w=320`，出厂照请求 `w=640`，Hero 大图编辑、作品设定图、领养封面与委托详情请求 `w=1280`。读取永久原图由管理员明确点击并发送 `original=1`；历史无参数委托原图链接保持兼容。所有响应继续 `no-store`，缩略处理失败不得回退并传输完整原图。

2026-09-09 用户追加授权回滚 PR #31 的浏览器 OSS 直读方案：既有图片接口恢复同源图片字节响应，
服务端通过 OSS SDK 读取永久原图或固定尺寸缩略结果，不返回 `delivery=url` 签名 DTO 或 302。
永久原图返回可信图片 MIME 与 `Content-Disposition: inline`，保留新窗口浏览器原生预览。
Hero 临时裁切预览继续检查自身到期时间与资产 READY 状态；邮件附件与工作单继续读取原始字节。
管理图片 GET 响应不包含私有对象路径或 OSS GET 签名；它们不进入公开 DTO、公开 HTML、日志或浏览器持久化存储。
图片请求与被动会话检查不更新活跃时间或续 Cookie；八小时闲置会话失效后必须重新登录，无周期性保活。
...
| 管理浏览器私有图片 GET | 管理 Host 同源接口，app 经 OSS SDK 读取并返回字节 |
...
- 作品竖版 `detail` 与 3:4 `work-card` 在所有响应式宽度保持单个居中水印，并统一按 480 px 参考宽度保持相同相对尺寸；后台公开水印预览与详情一致，设定图仍为左右双水印。
