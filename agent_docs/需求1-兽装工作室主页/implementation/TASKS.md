...
> **角色**：PLAN 的唯一可勾选实施分解。
> **状态**：T01–T21、GATE-06、GATE-07 与 EXT-02 已完成。T21 首次独立审查 findings、用户人工验收回归和完整门禁已收口；用户于 2026-08-02 明确确认 T21 完成。任务编号保留 T01–T53。

## 当前目标

GATE-07 与 T19–T21 已收口。下一任务为 T22，继续按契约先行原则补齐完整作品字段与约束。
...
- [x] **T21 · 第一垂直切片门禁**：从空库初始化管理员 → 创建作品 → 私有上传 → OSS 生成活动居中水印 → 发布 → 配置首页横竖轮播 → 新访客横/竖屏浏览 → 在 `/admin/site/branding` 更换一次 Logo 候选并原子再生成 → 验证旧 profile 在切换前持续可用、新 profile 完整切换 → 下架/精确清理。私有匿名读取失败、原图/Logo 候选无公开泄漏、公开图使用正确居中水印。_依赖：T20。_
  - 2026-08-02：首次独立审查为 NOT PASS；3 个 must-fix 与 1 个 should-fix 已完成实现者侧修复，等待未参与修复的审查者按报告条件重验。
  - 2026-08-02：用户人工验收追加 UI 回归修复：管理导航最终统一为“首页管理 → 全局水印 → 作品管理 → 修改密码”，各入口导航、一级标题、标签页标题同名；恢复 Hero 安全边距和作品筛选视觉；页脚邮箱/QQ 接入首页管理的版本化站点设置。
  - 2026-08-02：用户追加要求 `/admin/site/home` 直接预览已保存横竖轮播原图；复用既有同源鉴权 assetId 媒体接口完成，不改变独立的活动水印预览。`/api/public/v1/works/featured` 读取链已在 T20 完成，管理端“精选”写入仍按 T22 边界执行，本轮不提前进入 T22。
  - 2026-08-02：修复已启用轮播项仍显示“活动水印预览”并必然收到 409 的状态错配；已启用项改为当前公开水印说明，未启用项继续提供真实烘焙预览。
  - 2026-08-02：用户明确确认 T21 收口。首次 NOT PASS 结论继续作为历史审查事实保留；最终完成态以修复证据、人工验收结果和用户确认共同成立，不虚构第二份独立复审报告。
