...
页头是 sticky（1440×900 实测 73px）。`#hash` 跳转必须让开这段高度，否则目标标题被压在页头下方。
统一由 `html { scroll-padding-top: var(--public-anchor-offset) }` 提供，加在滚动容器上一次生效；
不要在各个目标元素上分别维护 `scroll-margin-top`（易漏且会与全局值叠加）。
...
- 首页集合 1–5 张 enabled 才公开就绪；委托集合不轮播，每方向同时只启用 1 张，且允许停用唯一启用图（先下架旧图再启用新图即替换），集合可以为空；
- 首页集合数量和顺序允许不同；委托集合无顺位概念；
- 不跨方向自动裁切或静默替代；
...
- 卡片默认使用 3:4 竖版 primary 出厂照。只有横版 `adoption_cover` 的领养作品回落到 16:9 横版
  封面；有竖版出厂照时仍优先出厂照。`/adoptions` 卡片以横版封面进入；封面缺失时以设定图进入
  同一 16:9 卡框——设定图公开变体仍是原比例 contain 生成，卡片层用 `object-fit: cover` 裁切
  显示，不再另生 16:9 变体；
...
- **单一媒体区**：出厂照 → 领养封面 → 设定图 合成同一个查看序列（成果图在前、参考图在后）。
  ...
  既然所有图片共用一个序列，也不再显示「出厂照 / 作品图集」这类分区标题；`aria-label="作品图集"`
  保留，屏幕阅读器仍能识别该区域。只有单张时缩略图行按长度自动隐藏；
...
- 合格 `adoption_cover`（横版单头成果照，用于 `/adoptions` 和首页当前领养）或合格
  `design_sheet`（完整设定图）至少其一：存在的那张（或两张）须 READY 且有 alt；封面缺失时
  公开端卡片、当前领养与详情封面位回落到设定图，同一张图不在详情图集重复出现；
...
- `design_sheet` 最多一张；单独一张设定图即可支撑发布，出厂照不能替代封面/设定图。
...
- 保留名称搜索、分页；搜索条收窄，右侧并排一个跳到 `/about#contact` 的「联系我们申请领养」按钮，
  与搜索输入框和搜索按钮同高（2.75rem）、同基线，构成一条操作带。领养需要人工沟通，因此联系入口
  与搜索同级可见；
...
`/admin/commissions` 默认 pending，支持 pending/accepted/rejected 三视图。列表使用白底行并以“昵称 · 物种”标记，只附提交时间、状态和回执；旧申请物种未知时明确标记待补录。列表 API 按状态返回该视图全量；查找（昵称/物种/回执编号）与分页均在客户端对已拉取数据完成，复用作品列表的分页组件。详情按需显示物种、手机号、QQ、身高、体重、备注和认证 no-store 图片预览，保存处理结果后给出可见成功反馈。
