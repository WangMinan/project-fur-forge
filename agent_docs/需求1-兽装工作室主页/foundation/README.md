...
- 作品页保持紧凑的图片优先节奏；低分辨率设定图与出厂照保留完整原图并在发布时生成各自不可变的私有 FFmpeg Lanczos 适配源，不以尺寸笼统阻断上传、保存或发布；全部管理端 FFmpeg 处理显示动态等待进度、阶段和已等待时间；
- 作品竖版详情与 3:4 卡片使用随输出宽度等比放大的单个居中水印；横版作品保持单居中，设定图保持左右双水印；
