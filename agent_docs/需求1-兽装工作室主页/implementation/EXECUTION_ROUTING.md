...
> **生效日期**：2026-08-04（T23–T25 已收口；T26–T27 服务端与前端接线完成；OQ-120 开放，等待正式文案、独立 Review 与用户验收）。
...
- T26–T27 服务端与前端（首页管理固定区块、`/commission` `/about` `/contact`）已完成；**OQ-120** 仍开放；两项任务仍未勾选，等待正式文案、独立 Review 与用户验收。
...
- T26/T27：后端与前端接线已完成；下一步由用户确认 **OQ-120** 正式文案并在首页管理录入，随后新上下文独立 Review；
