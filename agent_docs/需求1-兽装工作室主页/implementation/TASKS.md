...
> **角色**：当前唯一可勾选实施分解。
> **最后校准**：2026-08-07。
> **规则**：历史工程事实保留；任务只有在当前完成定义全部满足后才能勾选。用户验收由用户签署，实施者不能代签。

## 当前目标

阶段 C 与阶段 C.1 已完成人工验收，`GATE-C1` 已通过。项目当前进入 **阶段 D · P1 一期增强的范围确认**，优先讨论并实施返图墙垂直切片（T35–T36）。

最新 `main` 的 GitHub Actions 仍未全绿：`image-build` 成功，`checks` 在 `Production build` 失败，`e2e` 被跳过。该故障已从阶段 C 完成条件中移出，统一登记到阶段 E 的 T49，不阻断阶段 D。

媒体规则以 [`../requirements/MEDIA-PUBLICATION-POLICY.md`](../requirements/MEDIA-PUBLICATION-POLICY.md) 为唯一事实源。阶段 C 用户验收与 CI 移交记录见 [`notes/t34-c1/T34-C1-USER-ACCEPTANCE-2026-08-07.md`](./notes/t34-c1/T34-C1-USER-ACCEPTANCE-2026-08-07.md)。
...
- 正式域名、TLS、线上 Compose、升级、回滚和恢复演练仍延期到正式发布阶段；
...
> 当前范围尚在讨论。返图墙是已确认的最高优先级；T38–T40 在用户明确决定前不得自动实施。

- [ ] **T35 · 返图模型与可选授权记录**。_依赖：GATE-C1。_
- [ ] **T36 · 返图上传、轻量水印、管理与公开墙**。_依赖：T35。_
- [ ] **T37 · 展会掉落、当前展会与完整状态矩阵**。_依赖：GATE-C1。_
- [ ] **T38 · 受限站点文字内容扩展**：只增加预定义字段和分区，不建设万能 CMS。_依赖：GATE-C1；待范围确认。_
- [ ] **T39 · Slug 显式改址历史**：作品改 slug 后保留旧地址到新地址的永久重定向。_依赖：GATE-C1；待范围确认。_
- [ ] **T40 · 30 天回收站**：对选定可删除业务对象提供软删除、恢复与到期永久清理。_依赖：T36、T38；待范围确认。_
- [ ] **T41 · 手机轻量维护闭环**。_依赖：实际保留的 T36–T40 范围。_
- [ ] **T42 · P1 全链路门禁**。_依赖：阶段 D 最终确认范围。_
...
阶段完成与正式上线就绪是两个不同结论：阶段 C 已完成，但 T49、T50、T52 未完成前不得声称 GitHub Actions 已全绿或正式环境已经可发布。
