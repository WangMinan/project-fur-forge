...
| `ENGINEERING_PRIMARY` | 数据库、迁移、认证、安全、OSS、事务、服务端契约、公开投影与运维验证 | T19/T20 已收口，GPT 5.6 Sol |
| `REVIEW` | 对代码、截图、性能、安全、媒体和任务证据做独立复核 | T21 下一执行者，必须未参与 T19/T20 实现 |
...
1. 本轮实现者只准备 `implementation/notes/T21-REVIEW-PREP.md`，不勾选 T21、不代写通过结论；
2. 未参与 T19/T20 实现的 `REVIEW` 从空库和真实 OSS 重放 T21 完整链路，并复核代码差异、页面、日志、截图与清理证据；
3. 发现问题先登记 findings，修复后由独立审查者重验；
4. 独立审查证据完成后再交用户 `ACCEPTANCE`，其前不得进入 T22。
