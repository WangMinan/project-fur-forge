...
- 网页与管理端的 Display、Body、Metadata、UI、Legal 统一使用现有无衬线字体角色，界面字体栈不再显式引用宋体或微软雅黑；首页代表作品与设定领养的角色名、物种分别共享字体、字号和字重并统一左对齐，领养物种使用次级文字色且 `<1024px` 隐藏标签与事实区上分割线；Hero 拼贴字体与委托制作单 Noto Serif SC / PDF 链路保持不变。
