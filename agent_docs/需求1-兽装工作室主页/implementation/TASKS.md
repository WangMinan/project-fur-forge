...
> **角色**：需求1历史任务与状态权威。
> **最后校准**：2026-08-21。
> **状态**：需求1已关闭，不再作为活跃 backlog。
> **规则**：`[x]` 表示已有相应证据；`[-]` 表示按产品决策关闭、不再执行，不代表功能、独立 Review、用户验收或生产操作曾完成。

## 历史目标与阶段边界

以下内容保留为历史实施计划。自 2026-08-21 起不再从本文件启动工作；运行与部署安全基线仍由媒体策略和部署 Runbook 约束。

当前进入：

> **阶段 E · 完成全部产品与上线基线开发并冻结可上线 SHA。**
>
> T46 访问统计 → T51/T51-F1～F9 公开体验与媒体修复 → T52-E1～E6 生产媒体与部署能力 → T49 同一 SHA CI/独立 Review → T50 最终受控环境回归 → GATE-E。
...
_依赖：T51-F3；完成后进入 T51-F5。_

### T51-F5 · 竖版详情水印与作品卡/后台预览一致性修复

- [x] 竖版 `studio_photo` `detail` 与 3:4 `work-card` 统一使用 480 px 水印参考宽度，保持单个居中水印和相同相对尺寸；
- [x] 后台公开水印预览复用同一配方并与公开详情一致，不增加 CSS 叠层或第二套预览逻辑；
- [x] 参考宽度变化进入 `recipe-v3` 身份并生成新不可变 Key；横版详情、设定图左右双水印和旧配方整体回退不变；
- [x] 完成定向 integration、lint/typecheck/build 与真实作品浏览器回归；用户确认视觉正常，实现自测与用户确认均不代签 T49-R1 独立 Review。
_依赖：T51-F4；完成后由 T49-R1 对最新 SHA 重新门禁。_

### T51-F6 · 公共页脚备案居中与三栏重排

- [x] 宽屏页脚改为留白均衡的品牌、中间导航/备案、右侧法务三块结构；
- [x] ICP 备案号移入中间导航下方并继续链接工信部备案系统，公安备案继续使用既有配置槽且未完成时不显示虚假占位；
- [x] 右侧收敛为版权声明与“服务条款 / 隐私政策 / 开源软件声明 / 设计署名”两行；
- [x] 保持登录页 `brandOnly` 备案投影和窄屏安全换行，不改变运行时备案 Schema。
_依赖：T51-F5；本次为用户明确要求的局部页脚修复，远端 CI 结果由用户跟进。_

### T51-F7 · 管理私有预览与首页 Hero 4K 配方

- [x] 管理列表/卡片统一使用 `w=320`，编辑器较大预览统一使用 `w=640`；
- [x] 永久原图只允许显式 `original=1` 查看/下载，继续 `no-store`，非法参数和缩略失败不得回退原图；
- [x] 站点公开配方升级为不可变 `site-display-v2`，首页横版增加 2880×1620、3840×2160，Hero WebP 质量为 90；
- [x] 公开 SourceSet 优先完整 v2、整体回退完整 v1，普通屏幕继续选择 1280/1920 等较小档位；
- [x] 增加默认 dry-run、幂等的 `pnpm media:upgrade-site-display-v2` 与发布镜像同参数 ops 子命令，文档给出远端一次性容器命令；
- [x] 完成 Schema 前向迁移、unit/integration、CLI 参数透传、管理 URL 与必要 lint/typecheck/build 门禁；实现自测不代签 T49-R1 独立 Review。
_依赖：T51-F6；完成后由 T49-R1 对最新 SHA 重新门禁。_

### T51-F8 · 全局水印流程与首页精选排序优化

- [x] branding 新 UI 不再调用或显示真实水印预览；Logo、参数和 branding 版本一次确认即可创建/复用 profile 并启动持久全站刷新；
- [x] 移除 `WATERMARK_PREVIEW_REQUIRED` 应用门槛，空库直接激活；已有作品切换、首次失败、切换前失败和切换后清理重试保持原子/可恢复；
- [x] `/admin/works` 增加“全部作品 / 首页精选”Tab，删除列表和编辑器的数字排序输入，提供置顶、上移、下移、置底与前 12 状态；
- [x] 新增精选完整集合 GET/PUT 契约和单事务重排，校验 ID 唯一、集合一致与资源版本；冲突稳定返回 `FEATURED_ORDER_CONFLICT` 且不部分覆盖；
- [x] 新增精选自动追加、取消压紧和历史序号归一化；数据库保持连续 `0..n-1`，不新增迁移、唯一索引或拖拽依赖；
- [x] 首页已发布精选上限统一为 12，保留横向轨道与懒加载；完成 unit/integration/E2E、三视口、键盘/焦点/减少动效及必要 lint/typecheck/build 门禁；实现自测不代签 T49-R1 独立 Review 或用户验收。
_依赖：T51-F7；本轮按用户明确要求直接在 `main` 串行实施且不创建 PR，该例外不授权提交、推送或发布；完成后由 T49-R1 对最新 SHA 重新门禁。_

### T51-F9 · 首页与管理端动效、移动导航

- [x] `/admin/site/home` 的首页/委托 placement 都使用稳定 ID 的 FLIP move 动画，草稿卡 key 稳定；Tab 与内容面板有轻量过渡且不改变完整顺序提交、版本冲突和启停规则；
- [x] 管理导航使用单一 `ADMIN_NAV_ITEMS`；`<1024px` 使用八入口全屏抽屉，`1024–1279px` 保留横向导航，`>=1280px` 保留侧栏，窄屏用户信息/退出登录进入抽屉底部；
- [x] 公开与管理抽屉复用焦点陷阱、Escape、背景 `inert`、滚动锁定、路由关闭和焦点归还；面板与菜单项错峰进场并遵守 reduced-motion；
- [x] Hero 文案/控件首次加载依次入场；精选、委托投递、当前领养、最新动态首次入屏且不重复，SSR/无 JavaScript 默认完整可见；
- [x] 作品、领养和业务入口卡片仅在精细指针设备上轻微抬升；不可点击动态卡不制造 hover 暗示；新增动效只使用 transform/opacity/颜色/阴影且 reduced-motion 关闭位移与错峰；
- [x] 完成首页/委托重排、管理抽屉 1023/1024 边界与无障碍、公开动效/no-JS 自动化，以及双 Host、390/768/1440 和 1023/1024 px 浏览器验收；同步 STATE、设计入口、产物索引与实施记录。
_依赖：T51-F8；用户要求直接在 `main` 串行实施且不创建 PR，并于 2026-08-15 追加授权提交、推送；本轮不授权发布，完成后仍由 T49-R1 对最新 SHA 重新门禁。_
...
- [x] 证书监控只针对 ESA 托管边缘证书与实际公开 Host；宿主机必须验证无 443、无活动 ACME 服务/定时器/进程、Nginx 不引用证书、config test/reload 和 HTTP origin 可用；停用 unit 与未引用的历史账户/证书文件不阻断、不删除；
...
_依赖：T46、T51、T51-F1～F6、T52-E1～E6。_
...
- [-] 新上下文独立 Review preflight、Nitro 产物、Docker 守卫及停止/恢复
  边界，并覆盖本次 CORS/旧对象判定调整、T51-F5 水印对齐、T51-F6 页脚布局、T51-F7 媒体升级、T51-F8 水印/精选流程和 T51-F9 首页/导航动效；实现者不得代签。
- [-] Review 通过后重新发布不可变镜像，保存新的
  `repository@sha256:digest`，远端不得继续使用旧镜像。
_依赖：包含 T51-F9 的最新实现 SHA；完成后才可继续 T50/GATE-E 或远端 live preflight。_
...
- [-] ESA NS/边缘 TLS/HTTP origin、精确 Host、源站保护、宿主机无活动 ACME/443 和安全 reload 基线均已记录；
...
- [-] 复核 Nginx `1.30.4` 的 HTTP-only 配置、systemd 状态、Nginx 不引用证书、无活动 acme.sh/Certbot/续期服务或进程、80 监听和 443 关闭；停用 unit 与未引用的历史账户/证书文件可以保留；
