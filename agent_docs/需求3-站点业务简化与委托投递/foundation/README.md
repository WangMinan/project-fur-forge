...
> **基线**：继承需求1的双 Host、私有媒体、OSS/ESA、安全、部署与验收基线；继承需求2仍适用的官方渠道、二维码和名称搜索。
> **覆盖**：本目录对品牌英文名、返图墙、最新动态、Hero、作品字段、领养方式和委托入口的明确规定覆盖旧产品行为；未明确改变的需求1基线继续有效。
> **校准**：2026-08-15 文档复查后修订。
...
2. 英文品牌名恢复为 `DITE DOG`；
3. 公开端增加有节奏的导航、页面和图片动效；
4. 首页与委托页横版/竖版 Hero 完全独立维护；
5. 作品只维护名称、物种、图片和必要内部发布字段；
6. 领养使用独立横版单头成果图；
7. 新增 `/commission/apply` 与 `/admin/commissions`。
...
- 生产私有 Bucket CORS 必须精确允许公开站 Origin 执行条件 PUT，并继续允许管理站 Origin；不得使用通配 Origin。
- 匿名请求必须校验 Origin、Content-Type、body size、token、TTL、限流、蜜罐和一次性消费。
...
- 任何对象删除未完成时不得执行退役数据库 contract。
- 旧应用管理备份在新的净化备份完成并恢复验证后删除，不得提前牺牲全部数据库恢复能力。
- 退役 contract 完成后旧镜像不保证兼容，故障使用新镜像前向修复。
- 后续作品字段 contract 与返图/动态立即退役是两个不同迁移单元，不得混为一次巨型停机操作。
...
- 不删除 `commission_email_action`、旧 contact 兼容列或其它未被用户明确授权的数据。
