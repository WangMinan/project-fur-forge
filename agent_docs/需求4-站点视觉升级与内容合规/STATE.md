...
- 网页与管理端的 Display、Body、Metadata、UI、Legal 统一使用现有无衬线字体角色，界面字体栈不再显式引用宋体或微软雅黑；首页代表作品与设定领养的角色名、物种分别共享字体、字号和字重并统一左对齐，领养物种使用次级文字色且 `<1024px` 隐藏“角色选择”栏、物种标签与事实区上分割线，768～1023px 的名称区与媒体保留独立间距；Hero 拼贴字体与委托制作单 Noto Serif SC / PDF 链路保持不变。
...
- 首页自设委托幕在 `767px` 以下竖屏不再强制接近一屏高或垂直居中 stage；当前按内容高度排列，标题线到 stage 与行动后的留白均已收紧。
...
- T47-F6 删除首页与委托页硬编码 slogan，用现有竖版素材的受限 `4:5` 裁切修复 768×1024 与 390×844 的委托页首屏动线，并按后续复核收紧首页委托竖屏上下留白；未新增字段、媒体派生、断点组件或依赖。
