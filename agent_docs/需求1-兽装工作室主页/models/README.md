...
T12 已于 2026-07-31 建立 P0 Drizzle Schema、两项初始领域迁移、SQLite 约束/触发器和显式公开/管理媒体投影；S2 Review 收口新增第三项增量迁移，补齐媒体 role/usage 矩阵与 `source_variant_id` 处理来源关系。T22 已于 2026-08-03 把预留作品列接入三用途共享联合类型、管理 service/API/UI 和公开投影；T23–T27 已于 2026-08-04 收口。0012 补齐营业状态映射约束和受限站点内容列，0014 写入用户确认的 OQ-120 初始化默认值；T27-F1 的 0015 只新增隐私政策纯文本列及默认值，编辑入口仍位于独立“文案配置”。
...
- `site_content`：在既有首页设置上增加明确 nullable 字段：委托短说明、人工估价说明、邮件行动、受限 FAQ、工作室事实、制作范围、服务条款纯文本、隐私政策纯文本、公开抖音和防诈骗文字。服务条款沿用 `basic_terms` 历史列以保留既有数据，隐私政策使用 `privacy_policy`；FAQ JSON 只接受固定 `{ question, answer }[]`。其他页面内容不是自由 JSON，首页轮播媒体和水印 profile 也不进入该表。
...
- 工作室官方邮箱 `3114559925@qq.com`、QQ `3114559925` 与抖音 `to3114559925` 是版本化 `site_content` 公开渠道，不属于 `works` 私有联系人字段；关于/联系区等固定页面可渲染适用渠道，页脚不再重复联系方式，公开投影仍不得读取或返回作品私有联系人。

### T26–T27 营业状态与固定内容

- 委托与领养状态按 kind 独立创建/更新；不存在时以 `expectedVersion = 0` 创建，存在后按行版本更新，陈旧版本返回 409；href 由 kind 固定映射。
- 固定内容共用 `site_content.version`；字段继续允许 nullable，以支持管理员主动隐藏整区。0014 只为 NULL 字段和缺失状态写入已确认的 OQ-120 默认值；0015 只为新增 `privacy_policy` 的 NULL 写入用户授权默认值，两者都不覆盖已有内容。
- 所有文本只按纯文本输出；服务端拒绝超长值、HTML 标签、iframe、脚本协议和 Markdown 脚本链接。服务条款与隐私政策使用可换行纯文本，不解析 Markdown 或富文本。
