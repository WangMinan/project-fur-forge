...
> **状态**：2026-08-22 仅阶段 E 开放；T37～T46-F8 当前工程实现/用户连续实画面修正已记录，T47 真实手机/连续性能与王旻安/景宸视觉验收仍未执行。原阶段 F 已关闭，本文不补签独立 Review 或生产事实。
...
**修复**：当时 design/SPEC/PLAN/TASKS/STATE 先把五视口的一屏表达写成“标题、角色、名称/物种、状态、唯一行动”；用户后续将其修订为目录与当前角色两个行动，当前权威以 SPEC/TASKS 新口径为准。图片继续限制高度、居中留白或使用桌面图文编排，不把 caption 压到角色脸部。
...
- 任意浮点焦点通过 service/DTO 原样保留；本条当时验证的九宫格已被用户 T40-F1 替换，当前 UI 通过画面拖动与 0.1% 滑杆调整，不再量化预设。
...
## 15. 2026-08-21 用户新增交互修正

- 用户要求参考 `lingxun.me` 的逐幕 wheel。只读核对其实际实现为桌面监听 wheel、阻止默认滚动、按 section 索引 `scrollIntoView(smooth)` 并锁 600ms；本项目采纳章节索引思路，但使用自身 `min-width: 1024px` PC 边界，1023px 以下不拦截。
- 代表作品拆为 lead 与继续浏览两个停靠点；其它首页幕各一个，Footer 为末端。全站和普通路由不扩散该行为。
...
## 18. 2026-08-22 T46-F8 代表作品标题、留白与详情切换修正

- 用户选择把代表作品标题与自设委托、设定领养统一放在左上。三幕现复用同一 `.home-scene-heading` CSS，标题字阶、字体与行高的真实浏览器计算值一致；代表作品右侧只保留“更多角色与制作细节，请前往完整作品展示。”和唯一目录按钮。
- 双图经过连续实画面反馈后收敛为桌面最大 360×480，1440px 宽视口间距 56px。1440×665 短视口中代表作品幕高 654px，图片底边距幕底 32px；1440×900 中图片底边距幕底 119px，均无横向溢出。
- 卡顿根因是所有代表作品链接都带 `view=home-featured`，但旧实现只给第一张图片绑定 `home-featured-media`，点击第二张时仍会把第一张竖图以 720ms 共享对象形变到详情舞台。当前保留入口 query 与返回语义，代表作品改为直接路由切换；委托和领养共享媒体路径不受影响。
- 最终验证：`check:fast` 52/52 文件、313/313 项，smoke 10/10，production build/content guard 通过；Chrome 覆盖 390×844、430×932、768×1024、1023×900、1024×900、1440×900 和补充的 1440×665，无正向水平溢出、无 console error。第二张图片进入详情保留正确 URL，检测到的 View Transition 动画数为 0。详细记录见 [`../implementation/notes/2026-08-22-T46-F8-FEATURED-LAYOUT-TRANSITION.md`](../implementation/notes/2026-08-22-T46-F8-FEATURED-LAYOUT-TRANSITION.md)。
- T47 真实手机、连续性能与景宸/王旻安最终视觉验收仍保持开放；本节不代签远程 CI、部署或生产状态。
