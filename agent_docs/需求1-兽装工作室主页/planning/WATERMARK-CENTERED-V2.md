...
- 新增 `brand-centered-v2`，默认 `g=center`、`t=50`、水印图预处理 `resize,P_60`。
- OSS 处理顺序固定为：先把主图生成目标输出尺寸，再对水印图按目标图比例预处理，随后居中叠加并转换最终格式。
...
水印源 Object 完整名称使用 URL-safe Base64 编码。v2 基线应形成等价于以下语义的处理：

```text
image/<目标图 resize/crop>
/watermark,image_<base64url(水印源?x-oss-process=image/resize,P_60)>,t_50,g_center
/<最终 format/quality>
```
