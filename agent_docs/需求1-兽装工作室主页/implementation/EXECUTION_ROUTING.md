...
> **生效日期**：2026-08-03（T22 用户验收完成，进入 T23）。
...
- 当前写入权交给 `BACKEND_PRIMARY` 启动 T23；范围只含多图关系、角色约束、完整画布与按需配方，不提前进入 T24/T25/T37。
...
- T23（当前）：GPT-5.6 Sol 完成设定图/出厂照关系、数量、顺序、主图、完整画布、按需 recipe 和活动水印约束；
- T24：用户选择前端模型完成管理端角色化快速编辑、真实同源预览和公开水印预览；
- T25：GPT-5.6 Sol 先完成常规领养发布契约，前端模型再完成 `/adoptions` 与统一详情媒体分区；
- 每项完成 Review 和记录后再进入下一项；直接在 `main` 上串行，不并行写同一批文件。
