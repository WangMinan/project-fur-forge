...
- 首页/委托横竖 Hero 四集合独立；首页每集合 1–5 张轮播，委托每方向同时只启用 1 张、允许下架替换。
...
- published adoption 至少一张合格横版 cover 或设定图（至少其一，可同时存在）；design sheet 最多一张。
...
13. **领养发布门禁为封面/设定图二选一**：2026-08-17 用户明确领养常见场景是只做了单头；2026-08-19 进一步放宽——published adoption 只要求合格 `adoption_cover` 或 `design_sheet` 至少其一（READY + alt），`studio_photo` 仍为 0..5 可选但不能单独支撑发布。`ADOPTION_COVER_REQUIRED` 退役，`ADOPTION_MEDIA_REQUIRED` 覆盖「封面与设定图都没有」。公开端 `/adoptions`、首页当前领养、精选与详情的封面位回落到设定图；封面与设定图同一张时详情不重复入图集。commission/showcase 仍必须有 primary 出厂照。
14. **详情前后导航删除**：`/works/{slug}` 不再提供上一件/下一件。它没有维护来路，从 `/adoptions` 进入并切换后返回目标会退化为 `/works`；按来路返回保留。
15. **混排排版为等高铺满**：2026-08-17 用户复核指出等宽混排「很难看、大面积空白」。首页精选与 `/works` 改为等高——同行卡片高度一致、宽度按 3:4 / 16:9 自身比例伸展、行宽铺满且右边缘对齐。卡片框比例恒等于公开变体比例，所以铺满不产生裁切。
16. **详情单一图集**：领养封面与设定图并入出厂照图集（成果图在前、参考图在后），不再是独立标题分区，主图位置因此只有一处。桌面为「左侧舞台 + 右侧定宽缩略图列」：缩略图贴住内容右边缘、位置与图片宽度无关，舞台吃掉剩余宽度使横图能铺满且不被截断。切换缩略图时主图交叉淡化。同轮用户要求删除「继续浏览」，`related` 一并从 DTO 移除。
17. **页面切换必须用 `pageTransition`**：2026-08-17 用户报告「先完整加载、再闪一下、然后才走动效」。根因是 layout 层自包 `<Transition :key="route.path">` 位于 `NuxtPage` 的 Suspense 之外，新页面已以 opacity 1 渲染后才补入场起始态。改为 `NuxtPage :transition`（Suspense 内部）后，入场从第一帧即为正确起始态；管理端不参与。
18. **图集无分区标题**：所有图片共用同一查看序列，「出厂照 / 作品图集」标题移除；`aria-label` 保留供屏幕阅读器识别。
19. **锚点必须让开粘性页头**：页头实测 73px，锚点让位统一由 `html` 的 `scroll-padding-top`（5.5rem）提供，不在各目标元素上分别写 `scroll-margin-top`（易漏且会与全局值叠加）。
20. **`/adoptions` 联系入口**：搜索条收窄，右侧并排「联系我们申请领养」按钮跳 `/about#contact`，与搜索控件同高同基线。领养需人工沟通，联系入口与搜索同级可见。
21. **验证纪律**：只跑「受影响 spec」会漏掉未运行的 spec（CI run 32045006958 的 `t08-selfcheck` 即因此漏过）。涉及公开 UI 结构变更时必须运行完整 `pnpm test:e2e`。
22. **委托页大图为单图可替换**：2026-08-19 用户明确委托页不轮播，横/竖集合同时只允许 1 张启用；与首页不同的是委托集合允许停用最后一张启用图（下架即替换），首页集合仍保留 1–5 张与 `HERO_LAST_ENABLED_ITEM`。管理端委托 Tab 不再显示顺位与排序按钮。
23. **水印重建必须覆盖领养封面**：2026-08-19 定位 `/adoptions` 变空的根因——`WATERMARK_REBUILD` 生成目标只含 `studio_photo`/`design_sheet`，清理阶段却删除全部旧 profile 的水印公开变体，已发布领养作品的 `adoption_cover` 公开图被删且不再生成。`adoption_cover` 已纳入重建目标；既有缺失数据通过重新应用水印或下架重发布修复。
24. **委托申请列表查找与分页**：`/admin/commissions` 列表 API 按状态返回该视图全量；查找（昵称/物种/回执编号）与分页均在客户端对已拉取数据完成，复用作品列表的分页组件与分页工具，状态 Tab 语义不变。
...
| E 混排排版复核 | FU-17～FU-20 工程与受影响门禁完成 | GATE-E-FU5 本地通过（lint/typecheck/unit 193/integration 204/production build/E2E 81/真实浏览器五视口 CDP 实测）；独立 Review/用户验收/生产未执行 |
| E 查看序列与切换动效 | FU-21～FU-24 工程与受影响门禁完成 | GATE-E-FU6 本地通过；独立 Review/用户验收/生产未执行 |
| E 分区标题、联系入口与 CI 修复 | FU-25～FU-28 工程与完整门禁完成 | GATE-E-FU7 本地通过（含完整 E2E 240/240，修复 CI run 32045006958 的 t08 失败）；独立 Review/用户验收/生产未执行 |
...
`0039_r3_d_works_contract.sql` 在执行前强制检查人工状态、领养封面和主出厂照三项门禁。FU-11 与 2026-08-19 的封面/设定图二选一放宽的都是运行时发布门禁，不改写 `0039`：数据库没有对应 trigger，因此只有 cover 或只有设定图的领养作品可以正常发布，但在生产执行 `0039` 之前不得发布这类作品，否则迁移会以 `R3_D_CONTRACT_BLOCKED_ADOPTION_COVER` / `R3_D_CONTRACT_BLOCKED_PRIMARY_STUDIO_PHOTO` 停止（见 DATA-MIGRATION §12.1）。`0041_r3_d_hero_work_fk.sql` 前向修复 SQLite 重建 works 后 Hero 兼容表外键被重定向的问题。fresh、既有库、失败停止、重入、外键和 integrity 均只在临时数据库验证；生产迁移必须等景宸完成真实记录判断且三项门禁归零后串行执行。
...
1. 景宸在生产只读清单逐条判断歧义领养状态、补录独立 cover/主出厂照或先下架；三项计数均为 0 前不得执行 `0039`。注意顺序：`0039` 执行完成前不要在生产发布只有封面或只有设定图的领养作品，否则该迁移会停止（DATA-MIGRATION §12.1）；
