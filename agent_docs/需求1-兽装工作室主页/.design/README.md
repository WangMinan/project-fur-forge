...
- Hero 文案与控件只在首次加载错峰入场；首页四个内容区只在首次入屏揭示，SSR/无 JavaScript 默认可见；可点击卡片仅在精细指针下轻微抬升，不给静态动态卡制造点击暗示；
...
- `/works` 与 `/adoptions` 按发布时间倒序；首页精选最多公开 12 件，`sort_order` 只影响首页精选并由服务端维护连续顺序；
...
- 品牌页不展示真实水印预览，只提供 Logo/参数与一次确认保存刷新；作品端真实预览继续作为可选检查；
- 作品管理使用“全部作品 / 首页精选”二级 Tab；精选只显示业务位次和置顶/上下/置底操作，不显示内部排序数字，移动动画必须尊重减少动效偏好；
- 首页/委托大图排序复用稳定 ID 的 FLIP 位移动画；管理导航 `<1024px` 为八入口全屏抽屉，1024–1279px 为横向导航，1280px 起为侧栏；
