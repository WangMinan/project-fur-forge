...
后台图片通过认证接口签发十分钟 OSS 公网 GET 地址，浏览器直接读取原图或 OSS 缩略图；
签名客户端复用 `OSS_UPLOAD_BASE_URL`，不可改为内网地址。公开 ESA 缓存不变，不需部署
边缘函数。图片地址及跳转响应均 `no-store`；被动会话检查及图片签名不延长八小时闲置登录。
上线验收应从管理员浏览器确认 GET 可达、图片能够内联显示、缩略参数生效且图片字节不再
经过 ECS；如 OSS 域名/响应头/权限阻止加载，按实际问题处理，不回退应用服务器中转。
本地 fake/浏览器测试不代签真实 OSS 兼容性或生产 CPU 降幅。
