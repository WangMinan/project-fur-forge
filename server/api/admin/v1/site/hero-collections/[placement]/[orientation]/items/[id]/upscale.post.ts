...
import {
  ...
  startHeroCollectionItemUpscale,
...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const operation = startHeroCollectionItemUpscale(
      sqlite,
      id,
      scope.placement,
      scope.orientation,
    ...
    event.waitUntil(runHeroCollectionItemUpscale(
      sqlite,
      getMediaStorage(),
      operation.operationId,
      adminSessionFor(event).user.id,
    ).catch(error => event.captureError(error, { tags: ['hero-collection-upscale'] })))
