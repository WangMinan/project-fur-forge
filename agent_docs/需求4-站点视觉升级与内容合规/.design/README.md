...
> **状态**：2026-08-21 阶段 E T37～T46-F4 当前实现与用户连续实画面反馈已同步；真实手机和最终人工视觉验收待执行。
...
- PC 两个停靠点都显示“代表作品”，用 `SELECTED WORK · 01/02` 眉题标明页次；编号不再孤立出现在角色 caption 中。移动端只显示第一处标题。
...
- 与自设委托、设定领养共用同一主媒体画框高度；代表作品 02 紧接 01，不用大段空白分隔。

角色感：

- lead 进入时图片与 caption 使用不同但连续的时序；
- fine pointer hover 可轻微上移、聚焦、倾斜；触控端改为按压反馈，不模拟 hover。
- 首页代表作品、自设委托和设定领养三张主媒体使用同一组 fine-pointer 聚焦幅度；移动端与 reduced-motion 不应用该 transform。
- hover 命中区只限图片本身；鼠标离开图片即以 state 时序回落，不延续到 caption/行动区。
...
- wheel 顺序固定为 Hero → 代表作品 01 → 代表作品 02 → 自设委托 → 设定领养 → Footer；向上按相反顺序。
- 每个停靠点占一个动态视口；代表作品因 lead/次级浏览分成两个停靠点，其它幕只占一个。
- Header 固定在视口顶部，离开 Hero 后切换为可读实底；滚动容器只保留一份 Header offset，当前幕直接接在 Header 下方。
- 代表作品 02 从固定 Header 下方重复同级章节标题，次级媒体高度约 56svh 上限，不把内容垂直居中在空白画布中央；左右轨道按钮位于作品下方。
...
- 非 hash 导航必须先把新页面定位到顶部再呈现；返回/前进恢复历史位置，hash 目标继续让开 Header。
...
- 首页 ≥1024px 的逐幕 wheel 顺序、反向、锁定、Footer 停靠，以及 1023px/768/移动端不拦截滚动；
