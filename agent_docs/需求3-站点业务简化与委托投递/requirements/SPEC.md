...
- 只显示名称、物种、图集；
- **单一媒体区**：出厂照与领养封面合成同一个图集，封面排在出厂照之后作为最后一张。不设独立的
  「领养封面」标题分区，因此主图位置只有一处，不在限宽居中与靠左之间跳动。只做了单头的领养作品
  图集内只有封面一张，缩略图行按长度自动隐藏；
- 主图与缩略图作为一组整体居中，缩略图紧贴主图右侧竖排（窄屏回落为单列、缩略图横排在下方）。
  主图尺寸由高度上限 `clamp(20rem, 100vh - 15rem, 46rem)` 决定，宽度 = 上限高度 × 图片自身比例，
  因此竖图不顶穿一屏、横图也能用足可用宽度；
