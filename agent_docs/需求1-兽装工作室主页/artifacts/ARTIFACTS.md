...
| 任务 | `implementation/TASKS.md` | 2026-08-01 当前 | T01–T18、GATE-06 与 EXT-02 已完成；T19 未启动 |
...
| T14–T18 联合收口 | `implementation/notes/t14-t18-ui/T14-T18-UI-REPAIR-2026-08-01.md` | 2026-08-01 已通过 | 管理端接线、阻塞修复、真实 OSS/浏览器证据和用户联合验收 |
| 状态 | `STATE.md` | 2026-08-01 已更新 | T14–T18 已通过用户联合验收，下一项为 T19 |

## 当前执行文档

| 类型 | 路径 | 当前效力 |
| --- | --- | --- |
| 执行责任路由 | `implementation/EXECUTION_ROUTING.md` | 2026-08-01 当前生效；记录 T14–T18 已收口与 T19 之后默认分工，不属于产品契约 |
...
- T14：独立上传会话、角色化私有原图 5 分钟 V4 条件 PUT、版本/归属门禁、精确取消/过期/重试、安全 DTO 与管理端接线已实现。
- T15：服务端 HEAD/摘要/签名/图片信息核验、20–30 MB 内嵌 FFmpeg 私有预处理源、asset 状态、处理重试与管理端状态反馈已实现。
- T16：确定性 `recipe-v1`、`brand-standard-v1` 基础水印、跨 Bucket 保存、公开对象核验和来源谱系已实现；最终参数留给 T51/EXT-01。
- T17：最小非领养作品 CRUD、出厂照全量关系替换、主图/排序/alt/焦点/裁切/水印角、公开安全预览和管理端编辑已实现。
- T18：发布检查、发布/下架操作、幂等与竞态保护、先移除公开投影再清理、精确补偿重试和生成进度反馈已实现。

尚未实施：T19 作品详情 SSR、后台首页轮播、横竖双源切换、领养/返图媒体区、Touch Icon 和正式跨素材水印参数校准。它们已进入后续任务契约，不得写成当前联合任务已完成。
