...
> **角色**：当前媒体公开行为的唯一事实源。
> **最后校准**：2026-08-09。
> **状态分层**：媒体配方、阶段 D 页面、第 7 节 Endpoint 分离、T52-E2 私有/BPA preflight 入口和 T52-E3 ESA 公开投影收敛已经落地；第 9 节的精确缓存撤销仍须在阶段 E 的 T52-E4 完成。阶段 F 不再改变媒体产品契约，主要填写真实值、执行云配置和运行冻结 preflight；所需独立运维辅助脚本按 TASKS 的 F 边界处理。未在目标环境执行的 live 检查不得描述为生产通过。
