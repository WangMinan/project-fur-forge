...
> **状态**：2026-08-21 阶段 E T37～T46 当前实现与用户连续实画面反馈已同步；真实手机和最终人工视觉验收待执行。
...
- 首屏 CSS 动画必须从首次绘制直接开始，不得先完整显示、再由 `requestAnimationFrame` 补 class 造成闪烁和重启动画。
...
- wheel 顺序固定为 Hero → 代表作品 lead → 继续浏览 → 自设委托 → 设定领养 → Footer；向上按相反顺序。
- 每个停靠点占一个动态视口；代表作品因 lead/次级浏览分成两个停靠点，其它幕只占一个。
- Header 固定在视口顶部，离开 Hero 后切换为可读实底；滚动容器只保留一份 Header offset，当前幕直接接在 Header 下方。
- “继续浏览”从固定 Header 下方开始，次级媒体高度提高到约 56svh 上限，不把内容垂直居中在空白画布中央。
- “继续浏览”标题与左右轨道按钮共用同一 scene 顶部 padding，保持在同一行，不让按钮悬在 Header 下方。
