...
> **生效日期**：2026-08-03（T23–T25 实现与自动化自检完成，等待正式素材浏览器全链和独立 Review）。
...
- T23–T25 实现与自动化自检已完成；真实浏览器已创建常规领养并验证缺设定图发布阻断，正式素材上传等待浏览器扩展文件权限。
...
- T24（实现与自动化自检完成，任务未勾选）：管理端角色化快速编辑、真实同源原图预览、活动水印预览、上传恢复和列表媒体摘要已接入；
- T25（实现与自动化自检完成，任务未勾选）：regular adoption 发布、`/adoptions` 横版设定图页面和统一详情媒体分区已接入；
- 当前等待正式素材浏览器全链，然后由新的 GPT-5.6 Sol 上下文独立 Review；后续仍直接在 `main` 上串行。
