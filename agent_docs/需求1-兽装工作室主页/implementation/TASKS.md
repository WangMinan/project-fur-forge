...
>
> T46 访问统计 → T51/T51-F1～F3 公开体验与素材能力 → T52-E1～E6 生产媒体与部署能力 → T49 同一 SHA CI/独立 Review → T50 最终受控环境回归 → GATE-E。
...
- [x] 私有适配源使用独立不可变配方身份，永久原图不覆盖，公开 `recipe-v2` 只消费验证完成的处理源；
...
- [x] 私有适配源使用独立不可变 `studio-photo-upscale-lanczos-v1` 身份，永久原图不覆盖，公开 `recipe-v2` 与卡片裁切消费验证完成处理源的实际尺寸；
...
_依赖：T52-E1～E6 工程基线；完成后进入 T49。_
...
_依赖：T46、T51、T51-F1～F3、T52-E1～E6。_
