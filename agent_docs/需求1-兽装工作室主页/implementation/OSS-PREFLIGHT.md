...
- 用应用权限执行私有对象条件 PUT、HEAD/GET、跨 Bucket 图片处理与精确删除，并确认 Bucket 枚举和越权 Key 前缀被拒绝；
...
- 使用官方 `@alicloud/esa20240910` SDK 验证 `DescribePurgeTasks` 与精确 file purge 可用，`ListSites` 及针对无效 `SiteId=0` 的站点删除探针必须返回权限拒绝。
...
不允许用全站、Host、目录或前缀刷新替代精确 file 刷新。预检验证凭据可执行上述两个 API，并以安全的站点枚举/无效 Site 删除负向探针确认无站点控制面权限。真实 DNS、证书、套餐或有效 Site 写操作不用于权限探测，避免在错误授权时真的改变生产配置。
