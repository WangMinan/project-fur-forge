...
> **状态**：2026-08-04 执行版。T01–T27、GATE-06、GATE-07、EXT-01 与 EXT-02 已完成；T26–T27 独立 Review 为 `PASS WITH FOLLOW-UP`，下一工程任务为 T28。
...
首页轮播启用即为原子发布，不另建一套重复的“已发布”布尔值。启用前必须验证：启用项数量 1–5、每项横竖资产均 READY、alt 非空、排序无冲突、可选关联作品已发布；两个方向都具备当前 `recipe-v2` 全部宽度的 PUBLIC READY WebP + fallback，usage、活动 `brand-centered-v2` profile、配置与 Logo 摘要、水印布局、输出摘要和字节数完整。生成期间旧公开投影不变，全部验证后才启用；失败不出现半套横竖资源。
...
- 当前管理导航固定为“首页管理 → 文案配置 → 全局水印 → 作品管理 → 修改密码”，五个入口的导航、一级标题和标签页标题同名；首页管理只保留口号、公开邮箱/QQ和专用轮播编辑器，委托/领养营业状态及 T26–T27 固定内容迁入独立“文案配置”。该入口复用既有固定字段和版本化 API，不建设万能 CMS、页面树或账号列表。
